...
export async function workflowRoutes(app) {
    ...
    app.get('/', async (req, reply) => {
        try {
            ...
            const result = await pool.query(`SELECT id, name, description, type, trigger_event, is_active, run_count, last_run_at, created_at, updated_at
         FROM workflows WHERE workspace_id = $1 ORDER BY created_at DESC`, [workspaceId]);
    ...
    app.post('/', async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO workflows (workspace_id, name, description, type, trigger_event, conditions, actions, config, is_active)
    ...
    app.get('/:id', async (req, reply) => {
        try {
            ...
            const result = await pool.query(`SELECT * FROM workflows WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
    ...
    app.put('/:id', async (req, reply) => {
        try {
            ...
            if (body.trigger_event !== undefined) {
                setClauses.push(`trigger_event = $${idx++}`);
            ...
            const result = await pool.query(`UPDATE workflows SET ${setClauses.join(', ')} WHERE id = $${idx} AND workspace_id = $${idx + 1} RETURNING *`, values);
            if (result.rows.length === 0)
                return reply.code(404).send({ error: 'Workflow not found' });
            return reply.send({ success: true, data: result.rows[0] });
        }
    ...
    app.delete('/:id', async (req, reply) => {
        try {
            ...
            if (!workspaceId)
                return reply.code(401).send({ error: 'Unauthorized' });
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const result = await pool.query(`DELETE FROM workflows WHERE id = $1 AND workspace_id = $2 RETURNING id`, [id, workspaceId]);
            if (result.rows.length === 0)
                return reply.code(404).send({ error: 'Workflow not found' });
    ...
    app.post('/:id/toggle', async (req, reply) => {
        try {
            ...
            if (!workspaceId)
                return reply.code(401).send({ error: 'Unauthorized' });
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const result = await pool.query(`UPDATE workflows SET is_active = NOT is_active WHERE id = $1 AND workspace_id = $2 RETURNING id, is_active`, [id, workspaceId]);
            if (result.rows.length === 0)
                return reply.code(404).send({ error: 'Workflow not found' });
            return reply.send({ success: true, data: result.rows[0] });
        }
    ...
    app.post('/import', async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO workflows (workspace_id, name, description, type, trigger_event, conditions, actions, config)
    ...
    app.get('/:id/export', async (req, reply) => {
        try {
            ...
            if (!workspaceId)
                return reply.code(401).send({ error: 'Unauthorized' });
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const result = await pool.query(`SELECT name, description, type, trigger_event, conditions, actions, config
         FROM workflows WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
            if (result.rows.length === 0)
                return reply.code(404).send({ error: 'Workflow not found' });
    ...
    app.get('/:id/executions', async (req, reply) => {
        try {
            ...
            const wf = await pool.query(`SELECT id FROM workflows WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
            if (wf.rows.length === 0)
                return reply.code(404).send({ error: 'Workflow not found' });
            const result = await pool.query(`SELECT * FROM workflow_executions WHERE workflow_id = $1 ORDER BY executed_at DESC LIMIT 100`, [id]);
