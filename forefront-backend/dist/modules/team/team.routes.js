...
export async function teamRoutes(fastify) {
    ...
    fastify.post('/invite', async (request, reply) => {
        try {
            ...
            // Check permissions
            const hasPermission = await teamService.checkPermission(workspaceId, invitedBy, 'canManageTeam');
            if (!hasPermission) {
    ...
    // Update member
    fastify.patch('/:memberId', async (request, reply) => {
        try {
            const workspaceId = request.user?.workspaceId;
            ...
            // Check permissions
            const hasPermission = await teamService.checkPermission(workspaceId, userId, 'canManageTeam');
            if (!hasPermission) {
                return reply.code(403).send({ error: 'Insufficient permissions' });
            }
    ...
    // Remove member
    fastify.delete('/:memberId', async (request, reply) => {
        try {
            const workspaceId = request.user?.workspaceId;
            const userId = request.user?.userId;
            if (!workspaceId || !userId) {
