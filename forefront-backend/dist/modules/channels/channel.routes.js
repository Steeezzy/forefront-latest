 ...
 */
import { channelSettingsService } from '../../services/channels/ChannelSettingsService.js';
import { emailChannelService } from '../../services/channels/EmailChannelService.js';
import { pool } from '../../config/db.js';
import { authenticate } from '../auth/auth.middleware.js';
const VALID_CHANNELS = ['whatsapp', 'instagram', 'messenger', 'email', 'web'];
export async function channelRoutes(fastify) {
    ...
    fastify.register(async function protectedApi(api) {
        ...
        api.put('/:channel/settings', async (req, reply) => {
            ...
            const data = req.body;
            const updated = await channelSettingsService.update(workspaceId, channel, data);
        ...
        api.post('/conversations/:id/takeover', async (req, reply) => {
         ...
         WHERE id = $2 AND workspace_id = $3`, [userId, id, workspaceId]);
            // Add system message
            await pool.query(`INSERT INTO messages (conversation_id, sender_type, content, message_type)
         VALUES ($1, 'system', 'An agent has taken over this conversation. Auto-reply is paused.', 'text')`, [id]);
            return reply.send({ success: true });
        });
        ...
        api.post('/conversations/:id/release', async (req, reply) => {
         ...
         WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
            await pool.query(`INSERT INTO messages (conversation_id, sender_type, content, message_type)
         VALUES ($1, 'system', 'Auto-reply has been re-enabled for this conversation.', 'text')`, [id]);
            return reply.send({ success: true });
        });
        ...
        api.get('/auto-reply/logs', async (req, reply) => {
            ...
            if (!workspaceId)
                return reply.code(401).send({ error: 'No workspace' });
            const { channel, limit = '50', offset = '0', escalated } = req.query;
         ...
         FROM auto_reply_logs WHERE workspace_id = $1`, [workspaceId]);
            return reply.send({
                data: result.rows,
