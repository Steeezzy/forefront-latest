...
export class ZapierProvider {
     ...
     */
    async registerWebhook(workspaceId, integrationId, webhookUrl, triggerEvent, flowId) {
        const result = await pool.query(`INSERT INTO zapier_webhooks (integration_id, workspace_id, webhook_url, trigger_event, flow_id, is_active)
       VALUES ($1, $2, $3, $4, $5, true)
       ON CONFLICT (integration_id, trigger_event) 
       DO UPDATE SET webhook_url = $3, flow_id = $5, is_active = true, updated_at = NOW()
       RETURNING id`, [integrationId, workspaceId, webhookUrl, triggerEvent, flowId]);
        return result.rows[0].id;
    }
    /**
     * Unregister a Zapier webhook
     */
    async unregisterWebhook(webhookId, workspaceId) {
        await pool.query(`UPDATE zapier_webhooks SET is_active = false WHERE id = $1 AND workspace_id = $2`, [webhookId, workspaceId]);
    }
    /**
     * Get all active webhooks for a workspace
     */
    async getWebhooks(workspaceId) {
        const result = await pool.query(`SELECT zw.*, i.status as integration_status
       FROM zapier_webhooks zw
       JOIN integrations i ON i.id = zw.integration_id
       WHERE zw.workspace_id = $1 AND zw.is_active = true
       ORDER BY zw.created_at DESC`, [workspaceId]);
        return result.rows;
    }
    /**
     * Fire a trigger event — sends POST to all matching Zapier webhook URLs
     */
    async fireTrigger(workspaceId, event, payload) {
        // Find all active webhooks for this event
        const result = await pool.query(`SELECT zw.id, zw.webhook_url
       FROM zapier_webhooks zw
       JOIN integrations i ON i.id = zw.integration_id
       WHERE zw.workspace_id = $1 
         AND zw.trigger_event = $2 
         AND zw.is_active = true
         AND i.status = 'connected'`, [workspaceId, event]);
        ...
        const promises = result.rows.map(async (row) => {
            try {
                ...
                // Log the fire
                await pool.query(`UPDATE zapier_webhooks SET last_triggered_at = NOW() WHERE id = $1`, [row.id]);
                if (response.ok) {
                    sent++;
                }
                else {
     ...
     */
    async generateIntegrationKey(workspaceId) {
        const crypto = await import('crypto');
        const key = `ff_zapier_${crypto.randomBytes(24).toString('hex')}`;
        return key;
    }
}
//# sourceMappingURL=zapier.provider.js.map