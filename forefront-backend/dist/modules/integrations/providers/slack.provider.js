...
export class SlackNotificationService {
     ...
     */
    async notify(workspaceId, eventType, eventData) {
        // Get Slack integration
        const result = await pool.query(`SELECT id, credentials, config, status FROM integrations
       WHERE workspace_id = $1 AND integration_type = 'slack' AND status = 'connected'`, [workspaceId]);
     ...
     */
    async sendTestNotification(workspaceId) {
        const result = await pool.query(`SELECT credentials, config FROM integrations
       WHERE workspace_id = $1 AND integration_type = 'slack' AND status = 'connected'`, [workspaceId]);
        if (result.rows.length === 0) {
