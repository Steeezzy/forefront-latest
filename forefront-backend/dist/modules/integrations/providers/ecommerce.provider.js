...
export class WooCommerceProvider {
    ...
    async findOrdersByEmail(email) {
        try {
            const response = await fetch(`${this.storeUrl}/wp-json/wc/v3/orders?search=${encodeURIComponent(email)}&per_page=10`, { headers: { 'Authorization': this.authHeader } });
            if (!response.ok)
                return [];
            const data = await response.json();
            return data.map((d) => ({
    ...
    async getCustomer(email) {
        try {
            const response = await fetch(`${this.storeUrl}/wp-json/wc/v3/customers?email=${encodeURIComponent(email)}&per_page=1`, { headers: { 'Authorization': this.authHeader } });
            if (!response.ok)
                return null;
            const data = await response.json();
...
export class BigCommerceProvider {
    ...
    async getOrder(orderId) {
        try {
            ...
            const response = await fetch(`https://api.bigcommerce.com/stores/${this.storeHash}/v2/orders/${orderId}`, {
                headers: {
    ...
    async findOrdersByEmail(email) {
        try {
            const response = await fetch(`https://api.bigcommerce.com/stores/${this.storeHash}/v2/orders?email=${encodeURIComponent(email)}&limit=10`, {
                headers: {
                    'X-Auth-Token': this.accessToken,
                    'Accept': 'application/json',
                },
...
export class PrestaShopProvider {
    ...
    async getOrder(orderId) {
        try {
            const response = await fetch(`${this.baseUrl}/orders/${orderId}?output_format=JSON`, { headers: { 'Authorization': this.authHeader } });
            if (!response.ok)
                return null;
            const data = await response.json();
            ...
            return {
                ...
                items: (order.associations?.order_rows || []).map((i) => ({
                    name: i.product_name,
...
export class WordPressProvider {
     ...
     */
    getWidgetSnippet(workspaceId, widgetDomain) {
...
export class EcommerceManager {
    async lookupOrder(integrationType, credentials, orderId) {
    ...
    async findOrders(integrationType, credentials, email) {
        const provider = this.getProvider(integrationType, credentials);
        if (!provider || !('findOrdersByEmail' in provider))
            return [];
        return provider.findOrdersByEmail(email);
    }
    ...
    getProvider(integrationType, credentials) {
...
//# sourceMappingURL=ecommerce.provider.js.map