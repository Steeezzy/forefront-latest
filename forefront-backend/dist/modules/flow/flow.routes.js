...
export async function flowRoutes(app) {
    // GET /api/flows?agentId=...
    app.get('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { agentId } = z.object({ agentId: z.string().uuid() }).parse(req.query);
            const result = await pool.query(`SELECT id, name, description, trigger_type, is_active, updated_at, created_at
                 FROM flows WHERE agent_id = $1 ORDER BY created_at DESC`, [agentId]);
    ...
    app.post('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO flows (agent_id, name) VALUES ($1, $2) RETURNING *`, [body.agentId, body.name || 'Untitled Flow']);
    ...
    app.put('/:id', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            values.push(id);
            const result = await pool.query(`UPDATE flows SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`, values);
            if (result.rows.length === 0)
                return reply.status(404).send({ error: 'Flow not found' });
            return reply.send({ flow: result.rows[0] });
        }
    ...
    app.post('/:id/test', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const body = z.object({
    ...
    app.get('/:id/executions', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const result = await pool.query(`SELECT id, status, started_at, completed_at, total_tokens_used, llm_cost_usd, error_message
                 FROM flow_executions WHERE flow_id = $1 ORDER BY started_at DESC LIMIT 50`, [id]);
    ...
    app.get('/executions/:execId', { preHandler: [authenticate] }, async (req, reply) => {
        try {
    ...
    app.post('/from-template', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO flows (agent_id, name, description, trigger_type, is_active, nodes, edges)
    ...
    app.post('/seed-defaults', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { agentId } = z.object({ agentId: z.string().uuid() }).parse(req.body);
            // Check if agent already has flows
            const existing = await pool.query(`SELECT COUNT(*) FROM flows WHERE agent_id = $1`, [agentId]);
            ...
            for (const template of flowTemplates) {
                await pool.query(`INSERT INTO flows (agent_id, name, description, trigger_type, is_active, nodes, edges)
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
                    agentId,
                    template.name,
                    template.description,
