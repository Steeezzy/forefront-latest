...
export async function actionRoutes(app) {
    ...
    app.get('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const result = await pool.query(`SELECT id, name, instructions, is_active, updated_at 
                 FROM lyro_actions 
                 WHERE agent_id = $1 
    ...
    app.post('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO lyro_actions (agent_id, name, instructions, ask_confirmation) 
                 VALUES ($1, $2, $3, $4) 
                 RETURNING *`, [agentId, name, instructions || null, askConfirmation]);
