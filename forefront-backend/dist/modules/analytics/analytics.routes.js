...
export async function analyticsRoutes(fastify) {
    ...
    fastify.get('/overview', async (request, reply) => {
        try {
            ...
            const dailyResult = await query(`SELECT 
         ...
         WHERE workspace_id = $1 AND date >= $2 AND date <= $3`, [workspaceId, from, to]);
            // Get time series data
            const timeSeriesResult = await query(`SELECT 
          date,
          total_conversations,
          ai_resolutions,
          escalations
         ...
         ORDER BY date ASC`, [workspaceId, from, to]);
            return reply.send({
                success: true,
                data: {
    ...
    // AI Support stats
    fastify.get('/ai-support', async (request, reply) => {
        try {
            const workspaceId = request.user?.workspaceId;
            if (!workspaceId) {
                return reply.code(401).send({ error: 'Unauthorized' });
         ...
         ORDER BY count DESC`, [workspaceId]);
            ...
            const unansweredResult = await query(`SELECT 
          question,
          frequency,
          ai_confidence
    ...
    fastify.get('/human-support', async (request, reply) => {
        try {
            const workspaceId = request.user?.workspaceId;
            if (!workspaceId) {
                return reply.code(401).send({ error: 'Unauthorized' });
         ...
         ORDER BY conversations_handled DESC`, [workspaceId]);
            return reply.send({
                success: true,
                data: result.rows,
            });
    ...
    fastify.get('/export', async (request, reply) => {
        try {
           ...
           ORDER BY c.created_at DESC`, [workspaceId, from || null, to || null]);
                data = result.rows;
            }
            return reply.send({
                success: true,
