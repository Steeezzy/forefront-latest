...
export class InboxService {
    async createConversation(data) {
        ...
        try {
            ...
            let visitorId = null;
            const visitorRes = await client.query('SELECT id FROM visitors WHERE workspace_id = $1 AND visitor_id = $2', [data.workspaceId, data.visitorId]);
    ...
    async getConversations(workspaceId, filters = {}) {
        const { status = 'all', assignedTo, channel, search, tags, priority, page = 1, limit = 20, } = filters;
    ...
    async getConversationById(conversationId, workspaceId) {
       ...
       FROM conversations c
       LEFT JOIN users u ON c.assigned_user_id = u.id
       WHERE c.id = $1 AND c.workspace_id = $2`, [conversationId, workspaceId]);
        return result.rows[0] || null;
    }
    async updateConversation(conversationId, workspaceId, data) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
    ...
    async addMessage(data) {
        ...
        try {
         ...
         WHERE id = $2`, [data.content.substring(0, 200), data.conversationId]);
            await client.query('COMMIT');
            return message;
        }
        catch (error) {
    ...
    async getMessages(conversationId, workspaceId) {
        // First verify conversation belongs to workspace
        const convRes = await query('SELECT id FROM conversations WHERE id = $1 AND workspace_id = $2', [conversationId, workspaceId]);
    ...
    async markMessagesAsRead(conversationId, userId) {
        await query(`UPDATE messages 
       SET read_at = CURRENT_TIMESTAMP 
       WHERE conversation_id = $1 
       AND sender_type != 'visitor' 
       AND read_at IS NULL`, [conversationId]);
        // Mark conversation as read
        await query('UPDATE conversations SET is_read = true WHERE id = $1', [conversationId]);
    }
    async assignConversation(conversationId, workspaceId, assignedTo, assignedBy) {
        const result = await query(`UPDATE conversations 
       SET assigned_user_id = $1, updated_at = CURRENT_TIMESTAMP
       WHERE id = $2 AND workspace_id = $3
       RETURNING *`, [assignedTo, conversationId, workspaceId]);
        if (result.rows.length === 0) {
            throw new Error('Conversation not found');
        }
