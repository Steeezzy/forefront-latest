...
export class UsageService {
    async incrementMessageCount(workspaceId) {
        // 1. Get Subscription & Plan
        const subRes = await query('SELECT plan_id, current_period_start FROM workspaces WHERE id = $1', [workspaceId]);
        const planId = (subRes.rows[0]?.plan_id || 'free');
        ...
        await query('INSERT INTO usage_logs (workspace_id, message_count) VALUES ($1, 1)', [workspaceId]);
        const countRes = await query('SELECT SUM(message_count) as total FROM usage_logs WHERE workspace_id = $1 AND recorded_at >= $2', [workspaceId, periodStart]);
    ...
    async getUsage(workspaceId) {
        const subRes = await query('SELECT plan_id, current_period_start, current_period_end, subscription_status FROM workspaces WHERE id = $1', [workspaceId]);
    ...
    async trackTokens(workspaceId, tokens) {
