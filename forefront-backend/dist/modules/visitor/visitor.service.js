...
export class VisitorService {
    async trackVisitor(data) {
        ...
        try {
            ...
            const existingRes = await client.query('SELECT id, total_visits, first_seen_at FROM visitors WHERE workspace_id = $1 AND visitor_id = $2', [data.workspaceId, data.visitorId]);
            let visitor;
            if (existingRes.rows.length > 0) {
                // Update existing visitor
                const existing = existingRes.rows[0];
                const updateRes = await client.query(`UPDATE visitors SET
           ...
           RETURNING *`, [
                ...
                // Create new visitor
                const insertRes = await client.query(`INSERT INTO visitors
    ...
    async trackPageView(data) {
        const result = await query(`INSERT INTO page_views (visitor_id, page_url, page_title, referrer, duration_seconds)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [data.visitorId, data.pageUrl, data.pageTitle, data.referrer, data.durationSeconds]);
        return result.rows[0];
    }
    async setVisitorOffline(visitorId) {
        await query(`UPDATE visitors SET is_online = false, last_activity_at = CURRENT_TIMESTAMP
       WHERE id = $1`, [visitorId]);
    }
    async getVisitorById(visitorId, workspaceId) {
        const result = await query(`SELECT v.*,
       ...
       FROM visitors v
       WHERE v.id = $1 AND v.workspace_id = $2`, [visitorId, workspaceId]);
        return result.rows[0] || null;
    }
    async getVisitors(workspaceId, options = {}) {
        ...
        const result = await query(`SELECT * FROM visitors ${whereClause}
       ORDER BY last_activity_at DESC
       LIMIT $${values.length + 1} OFFSET $${values.length + 2}`, [...values, limit, offset]);
        return result.rows;
    }
    async getOnlineVisitorsCount(workspaceId) {
        const result = await query('SELECT COUNT(*) FROM visitors WHERE workspace_id = $1 AND is_online = true', [workspaceId]);
