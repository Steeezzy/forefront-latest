...
export async function kbRoutes(app) {
    // GET /api/kb?agentId= — List all knowledge bases
    app.get('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { agentId } = z.object({ agentId: z.string().uuid() }).parse(req.query);
            const result = await pool.query(`SELECT * FROM knowledge_bases WHERE agent_id = $1 ORDER BY created_at DESC`, [agentId]);
    ...
    app.post('/', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO knowledge_bases (agent_id, name, description, embedding_model, embedding_dimensions, chunk_strategy, chunk_size, chunk_overlap)
    ...
    app.get('/:id', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            if (result.rows.length === 0)
                return reply.status(404).send({ error: 'KB not found' });
            const docs = await pool.query(`SELECT id, name, source_type, status, chunk_count, token_count, created_at FROM kb_documents WHERE kb_id = $1 ORDER BY created_at DESC`, [id]);
    ...
    app.post('/:id/documents', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            ...
            const docResult = await pool.query(`INSERT INTO kb_documents (kb_id, name, source_type, metadata) VALUES ($1, $2, $3, $4) RETURNING *`, [kbId, body.name, body.source_type || 'text', JSON.stringify(body.metadata || {})]);
    ...
    app.get('/:id/documents', { preHandler: [authenticate] }, async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
            const result = await pool.query(`SELECT * FROM kb_documents WHERE kb_id = $1 ORDER BY created_at DESC`, [id]);
    ...
    app.delete('/:id/documents/:docId', { preHandler: [authenticate] }, async (req, reply) => {
        try {
