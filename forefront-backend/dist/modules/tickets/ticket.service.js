...
async function generateTicketNumber(workspaceId) {
    const result = await pool.query(`SELECT COUNT(*) AS count FROM tickets WHERE workspace_id = $1`, [workspaceId]);
...
export class TicketService {
    ...
    async createTicket(data) {
        ...
        const initialStatus = data.assigned_to ? 'open' : 'unassigned';
     ...
     */
    async getTickets(workspaceId, filters = {}) {
        ...
        const [ticketsRes, countRes] = await Promise.all([
            pool.query(`SELECT t.*, u.email AS assignee_email
         FROM tickets t
         LEFT JOIN users u ON t.assigned_to = u.id
         WHERE ${whereClause}
         ORDER BY ${sortCol} ${sortDir}
         LIMIT ${limit} OFFSET ${offset}`, values),
            pool.query(`SELECT COUNT(*) AS total FROM tickets t WHERE ${whereClause}`, values),
        ]);
        return {
     ...
     */
    async getTicketById(ticketId, workspaceId) {
        const [ticketRes, commentsRes] = await Promise.all([
            pool.query(`SELECT t.*, u.email AS assignee_email
         FROM tickets t
         LEFT JOIN users u ON t.assigned_to = u.id
         WHERE t.id = $1 AND t.workspace_id = $2`, [ticketId, workspaceId]),
            pool.query(`SELECT * FROM ticket_comments WHERE ticket_id = $1 ORDER BY created_at ASC`, [ticketId]),
        ]);
     ...
     */
    async updateTicket(ticketId, workspaceId, data) {
        // If changing status, validate the transition
        if (data.status) {
            const current = await pool.query(`SELECT status FROM tickets WHERE id = $1 AND workspace_id = $2`, [ticketId, workspaceId]);
            ...
            if (!allowed?.includes(data.status)) {
                throw new Error(`Invalid status transition: ${currentStatus} → ${data.status}. Allowed: ${allowed?.join(', ')}`);
            }
        }
        const setClauses = [];
        const values = [];
        ...
        const result = await pool.query(`UPDATE tickets SET ${setClauses.join(', ')}
     ...
     */
    async assignTicket(ticketId, workspaceId, assignedTo, assignedBy) {
       ...
       RETURNING *`, [assignedTo, ticketId, workspaceId]);
        if (result.rows.length === 0) {
            throw new Error('Ticket not found');
        }
    ...
    async addComment(data) {
        const result = await pool.query(`INSERT INTO ticket_comments
       ...
       RETURNING *`, [
        ...
        await pool.query(`UPDATE tickets SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [data.ticket_id]);
        return result.rows[0];
    }
    /**
     * Resolve a ticket. Optionally triggers CSAT survey (handled by caller).
     */
    async resolveTicket(ticketId, workspaceId, resolvedBy) {
        return this.updateTicket(ticketId, workspaceId, { status: 'solved' });
    }
    /**
     * Convert a live conversation into a ticket.
     */
    async convertConversationToTicket(conversationId, workspaceId, subject, createdBy) {
        // Fetch conversation details
        const convo = await pool.query(`SELECT * FROM conversations WHERE id = $1 AND workspace_id = $2`, [conversationId, workspaceId]);
     ...
     */
    async getTicketStats(workspaceId) {
