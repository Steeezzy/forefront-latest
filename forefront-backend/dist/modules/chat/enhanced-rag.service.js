...
export class EnhancedRAGService {
    ...
    async resolveAIResponse(workspaceId, conversationId, userMessage, options = {}) {
        ...
        try {
            ...
            const { rows: agents } = await pool.query(`SELECT system_prompt, tone, name FROM agents WHERE workspace_id = $1 AND is_active = true LIMIT 1`, [workspaceId]);
            ...
            const result = await sarvamClient.chatCompletion(messages, {
                temperature: 0.7,
                max_tokens: 500,
            });
    ...
    calculateConfidence(chunks, query) {
        if (chunks.length === 0)
            return 0;
    ...
    buildMessages(agent, history, chunks, currentMessage) {
...
5. Answer in the same language as the user's question`;
    ...
    async logEscalation(conversationId, workspaceId, question, confidence) {
        try {
            await pool.query(`INSERT INTO ai_escalations (conversation_id, workspace_id, reason, ai_confidence, original_question)
         VALUES ($1, $2, $3, $4, $5)`, [conversationId, workspaceId, 'low_confidence', confidence, question]);
            // Also log as unanswered question for future training
            await pool.query(`INSERT INTO unanswered_questions (workspace_id, question, frequency, ai_confidence)
         VALUES ($1, $2, 1, $3)
         ON CONFLICT (workspace_id, normalized_question) 
         DO UPDATE SET frequency = unanswered_questions.frequency + 1, last_asked_at = CURRENT_TIMESTAMP`, [workspaceId, question, confidence]);
        }
        catch (error) {
    ...
    async suggestReplies(conversationId, workspaceId) {
        try {
            const history = await this.chatService.getMessages(conversationId);
...
Suggest 3 professional, helpful replies. Each reply should be concise (1-2 sentences max). Format as a JSON array of strings.`;
    ...
    async rewriteDraft(draft, tone = 'professional') {
        try {
