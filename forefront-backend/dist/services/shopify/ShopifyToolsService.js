...
export class ShopifyToolsService {
     ...
     */
    async executeTool(toolName, args, workspaceId) {
        switch (toolName) {
    ...
    async getOrderStatus(args, workspaceId) {
        ...
        if (res.rows.length === 0)
            return `Order ${args.order_number || args.order_id} not found.`;
        const o = res.rows[0];
        const items = (o.line_items || []).map((li) => `${li.title} x${li.quantity} (${currency}${li.price})`).join(', ');
    ...
    async getCustomerOrders(args, workspaceId) {
        const storeId = await this.getStoreId(workspaceId);
        if (!storeId)
            return 'No Shopify store connected.';
        const currency = await this.getStoreCurrency(storeId);
        const custRes = await pool.query(`SELECT shopify_id FROM shopify_customers WHERE store_id = $1 AND email = $2`, [storeId, args.email]);
        if (custRes.rows.length === 0)
            return `No customer found with email ${args.email}.`;
        const limit = args.limit || 3;
        const orders = await pool.query(`SELECT name, total_price, financial_status, fulfillment_status, created_at
       FROM shopify_orders WHERE store_id = $1 AND shopify_customer_id = $2 ORDER BY created_at DESC LIMIT $3`, [storeId, custRes.rows[0].shopify_id, limit]);
        if (orders.rows.length === 0)
            return `No orders found for ${args.email}.`;
        return orders.rows.map((o) => `${o.name} — ${currency}${o.total_price} — ${o.financial_status}/${o.fulfillment_status || 'not shipped'} — ${new Date(o.created_at).toLocaleDateString()}`).join('\n');
    }
    async trackShipment(args, workspaceId) {
        const storeId = await this.getStoreId(workspaceId);
        if (!storeId)
            return 'No Shopify store connected.';
        const orderNum = (args.order_number || '').replace('#', '');
        const orderRes = await pool.query(`SELECT shopify_id, name FROM shopify_orders WHERE store_id = $1 AND (order_number::text = $2 OR name ILIKE $3) LIMIT 1`, [storeId, orderNum, `%${orderNum}%`]);
        if (orderRes.rows.length === 0)
            return `Order ${args.order_number} not found.`;
        ...
        if (fulfillments.length === 0)
            return `Order ${orderRes.rows[0].name} has not been shipped yet.`;
        return fulfillments.map((f) => `Tracking: ${f.tracking_company || 'N/A'} — ${f.tracking_number || 'N/A'} — ${f.tracking_url || 'No URL'} — Status: ${f.status}`).join('\n');
    }
    async checkAvailability(args, workspaceId) {
        const storeId = await this.getStoreId(workspaceId);
        if (!storeId)
            return 'No Shopify store connected.';
        const currency = await this.getStoreCurrency(storeId);
        ...
        return res.rows.map((p) => {
            const variants = (p.variants || []).map((v) => `${v.title}: ${currency}${v.price} (${v.inventory_quantity || 0} in stock)`).join(', ');
            return `${p.title}: ${variants}`;
        }).join('\n');
    }
    async cancelOrder(args, workspaceId) {
        const storeId = await this.getStoreId(workspaceId);
        if (!storeId)
            return 'No Shopify store connected.';
        const orderNum = (args.order_number || '').replace('#', '');
        const orderRes = await pool.query(`SELECT shopify_id, name, financial_status, fulfillment_status FROM shopify_orders WHERE store_id = $1 AND (order_number::text = $2 OR name ILIKE $3) LIMIT 1`, [storeId, orderNum, `%${orderNum}%`]);
        if (orderRes.rows.length === 0)
            return `Order not found.`;
        const o = orderRes.rows[0];
        if (o.financial_status === 'refunded')
            return `Order ${o.name} is already refunded and cannot be cancelled.`;
        if (o.fulfillment_status === 'fulfilled')
            return `Order ${o.name} is already fulfilled and cannot be cancelled.`;
        // Call Shopify API
        const store = await this.getStoreInfo(storeId);
        if (!store)
            return 'Store config not found.';
        const client = new ShopifyApiClient(store.shop_domain, store.access_token);
        await client.cancelOrder(o.shopify_id, args.reason);
        await pool.query(`UPDATE shopify_orders SET financial_status = 'voided', cancelled_at = CURRENT_TIMESTAMP WHERE store_id = $1 AND shopify_id = $2`, [storeId, o.shopify_id]);
        return `Order ${o.name} has been cancelled successfully.${args.reason ? ` Reason: ${args.reason}` : ''}`;
    }
    async initiateRefund(args, workspaceId) {
        const storeId = await this.getStoreId(workspaceId);
        if (!storeId)
            return 'No Shopify store connected.';
        const currency = await this.getStoreCurrency(storeId);
        const orderNum = (args.order_number || '').replace('#', '');
        const orderRes = await pool.query(`SELECT shopify_id, name, line_items, total_price FROM shopify_orders WHERE store_id = $1 AND (order_number::text = $2 OR name ILIKE $3) LIMIT 1`, [storeId, orderNum, `%${orderNum}%`]);
        if (orderRes.rows.length === 0)
            return `Order not found.`;
        const o = orderRes.rows[0];
        const store = await this.getStoreInfo(storeId);
        if (!store)
            return 'Store config not found.';
        const client = new ShopifyApiClient(store.shop_domain, store.access_token);
        const lineItems = (o.line_items || []).map((li) => ({ line_item_id: String(li.id), quantity: li.quantity }));
...
//# sourceMappingURL=ShopifyToolsService.js.map