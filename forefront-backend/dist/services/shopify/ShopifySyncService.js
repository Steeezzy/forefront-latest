...
export class ShopifySyncService {
     ...
     */
    async initialSync(storeId, shopDomain, accessToken) {
        const client = new ShopifyApiClient(shopDomain, accessToken);
        const startTime = Date.now();
        // Create sync job
        const jobRes = await pool.query(`INSERT INTO shopify_sync_jobs (store_id, type, status) VALUES ($1, 'initial', 'running') RETURNING id`, [storeId]);
        ...
        try {
            ...
            await pool.query(`UPDATE shopify_sync_jobs SET status = 'completed', records_synced = $1, completed_at = CURRENT_TIMESTAMP WHERE id = $2`, [customersCount + productsCount + ordersCount, jobId]);
    ...
    // ─── Upsert Helpers ────────────────────────────────────────────────
    async upsertCustomer(storeId, raw) {
        const result = await pool.query(`INSERT INTO shopify_customers (shopify_id, store_id, email, first_name, last_name, phone,
       ...
       RETURNING id`, [
            ...
            raw.tags ? raw.tags.split(',').map((t) => t.trim()) : [],
            raw.note, raw.verified_email || false, JSON.stringify(raw),
            raw.created_at, raw.updated_at,
        ]);
        return result.rows[0].id;
    }
    async upsertOrder(storeId, raw) {
        const result = await pool.query(`INSERT INTO shopify_orders (shopify_id, store_id, shopify_customer_id, order_number, name,
       ...
       RETURNING id`, [
            ...
            (raw.discount_codes || []).map((d) => d.code),
            raw.note, raw.tags ? raw.tags.split(',').map((t) => t.trim()) : [],
    ...
    async upsertProduct(storeId, raw) {
        const result = await pool.query(`INSERT INTO shopify_products (shopify_id, store_id, title, vendor, product_type, handle,
    ...
    async upsertAbandonedCheckout(storeId, raw) {
        const result = await pool.query(`INSERT INTO shopify_abandoned_checkouts (shopify_id, store_id, email, phone,
     ...
     */
    async linkCustomerToContact(storeId, shopifyCustomerId) {
        const custRes = await pool.query(`SELECT id, email FROM shopify_customers WHERE store_id = $1 AND shopify_id = $2`, [storeId, shopifyCustomerId]);
        ...
        if (contactRes.rows.length > 0) {
            ...
            const cRes = await pool.query(`INSERT INTO contacts (workspace_id, email, shopify_customer_id, store_id) VALUES ($1, $2, $3, $4) RETURNING id`, [workspaceId, email, shopifyCustomerId, storeId]);
            contactId = cRes.rows[0].id;
        }
        await pool.query(`UPDATE shopify_customers SET contact_id = $1 WHERE id = $2`, [contactId, shopifyCustDbId]);
    }
    // ─── Private Helpers ───────────────────────────────────────────────
    async fetchCustomersPage(client, pageInfo) {
        // Use ShopifyApiClient's searchCustomers for initial load,
        // or getCustomers with page_info for pagination.
        try {
            const result = await client.getWithHeaders(`/customers.json?limit=250${pageInfo ? `&page_info=${pageInfo}` : ''}`);
            const nextPage = client.parseLinkHeader?.(result.linkHeader);
    ...
    async getShopDomainForClient(_client) {
        return '';
    }
    async updateJobProgress(jobId, recordsSynced) {
...
//# sourceMappingURL=ShopifySyncService.js.map