...
export class CopilotService {
     ...
     */
    async searchKB(query, workspaceId) {
    ...
    // ─── Reply Draft ───────────────────────────────────────────────────
    async generateReplyDraft(request) {
        ...
        const results = await this.searchService.searchByText(request.workspace_id, lastCustomerMsg.content, { top_k: 6, min_score: 0.65 });
        const reranked = await this.rerankerService.rerank(lastCustomerMsg.content, results, 3);
...
${transcript}`,
    ...
    // ─── KB Search for Agent ───────────────────────────────────────────
    async searchKBForAgent(request) {
        const lastCustomerMsg = [...request.messages]
            .reverse()
            .find((m) => m.role === 'user');
        if (!lastCustomerMsg)
