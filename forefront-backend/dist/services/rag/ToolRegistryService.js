...
export class ToolRegistryService {
    ...
    registerBuiltinTools() {
        ...
        this.register({
            ...
            handler: async (args, context) => {
                ...
                const results = await searchService.searchByText(context.workspace_id, args.query, { top_k: 3, min_score: 0.65 });
        ...
        for (const st of shopifyTools) {
            this.register({
                ...
                parameters: Object.entries(st.parameters.properties).map(([key, prop]) => ({
                    name: key,
                    type: prop.type,
                    description: prop.description,
                    required: st.parameters.required?.includes(key) || false,
                })),
                handler: async (args, context) => {
        ...
        this.register({
            ...
            handler: async (args, context) => {
                const result = await pool.query(`INSERT INTO tickets (workspace_id, subject, description, priority, source, status)
        ...
        this.register({
            ...
            handler: async (args, context) => {
                await pool.query(`INSERT INTO email_queue (workspace_id, to_address, subject, body)
           VALUES ($1, $2, $3, $4)`, [context.workspace_id, args.to, args.subject, args.body]);
    ...
    registerWorkspaceTool(tool) {
        if (!tool.workspace_id)
            throw new Error('workspace_id required for workspace tools');
     ...
     */
    getOpenAIToolSchema(workspaceId) {
