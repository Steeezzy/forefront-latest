...
export class GuardrailsService {
    ...
    async loadRules(workspaceId) {
        ...
        const result = await pool.query(`SELECT * FROM guardrail_rules
       WHERE workspace_id = $1 AND enabled = true
       ORDER BY priority ASC`, [workspaceId]);
     ...
     */
    async evaluate(input, workspaceId) {
        ...
        for (const rule of rules) {
            if (rule.type === 'confidence_gate')
                continue; // only evaluated on output
     ...
     */
    async evaluateOutput(output, confidence, workspaceId) {
        const rules = await this.loadRules(workspaceId);
        const triggeredRules = [];
        let finalAction = 'allow';
        let rephrased;
    ...
    checkRule(rule, text) {
        ...
        switch (rule.type) {
            case 'topic_block': {
                const topics = rule.config.topics || [];
            ...
            case 'pii_detection': {
                const detectTypes = rule.config.detect || ['email', 'phone', 'credit_card'];
                return detectTypes.some((type) => {
    ...
    async createRule(rule) {
        const result = await pool.query(`INSERT INTO guardrail_rules (workspace_id, name, type, config, action, priority, enabled)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [rule.workspace_id, rule.name, rule.type, JSON.stringify(rule.config), rule.action, rule.priority, rule.enabled]);
    ...
    async updateRule(id, updates) {
        ...
        values.push(id);
        const result = await pool.query(`UPDATE guardrail_rules SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`, values);
        if (result.rows.length === 0)
            throw new Error('Guardrail rule not found');
        this.invalidateCache(result.rows[0].workspace_id);
        return result.rows[0];
    }
    async deleteRule(id) {
        const result = await pool.query(`DELETE FROM guardrail_rules WHERE id = $1 RETURNING workspace_id`, [id]);
        if (result.rows.length > 0) {
            this.invalidateCache(result.rows[0].workspace_id);
        }
    }
