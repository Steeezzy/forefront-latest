...
export class HandoffService {
     ...
     */
    shouldHandoff(session, latestResponse) {
        // 1. Low confidence
    ...
    async createHandoffEvent(params) {
        ...
        const result = await pool.query(`INSERT INTO handoff_events
    ...
    async acceptHandoff(handoffId, agentId) {
        const result = await pool.query(`UPDATE handoff_events
       SET status = 'accepted', assigned_agent_id = $1, accepted_at = CURRENT_TIMESTAMP
       WHERE id = $2 AND status = 'pending'
       RETURNING *`, [agentId, handoffId]);
     ...
     */
    async getPendingHandoffs(workspaceId) {
        const result = await pool.query(`SELECT * FROM handoff_events
     ...
     */
    async getHandoffsByConversation(conversationId) {
        const result = await pool.query(`SELECT * FROM handoff_events WHERE conversation_id = $1 ORDER BY created_at DESC`, [conversationId]);
        return result.rows;
    }
    /**
     ...
     */
    async findAvailableAgent(workspaceId) {
