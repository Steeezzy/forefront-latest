...
export class RerankerService {
     ...
     */
    async rerank(query, results, topN = 5) {
        if (results.length === 0)
            return [];
     ...
     */
    async cohereRerank(query, results, apiKey) {
        try {
     ...
     */
    calculateBM25(queryTokens, chunkTokens) {
     ...
     */
    buildContext(results, maxTokens = 3000) {
        if (results.length === 0)
            return '';
        let context = '';
        let tokenEstimate = 0;
