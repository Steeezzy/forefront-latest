...
export class NodeExecutorRegistry {
    ...
    register(subtype, executor) {
        this.executors.set(subtype, executor);
    }
    async execute(subtype, config, variables, context) {
    ...
    registerBuiltinExecutors() {
        ...
        const triggerPassthrough = async (config, variables) => ({
        ...
        this.register('based_on_variable', async (config, variables) => {
            ...
            switch (config.operator) {
                case 'equals':
                    result = String(value) === String(config.value);
                    break;
                case 'not_equals':
                    result = String(value) !== String(config.value);
                    break;
        ...
        this.register('context_builder', async (config, variables) => {
            ...
            return {
                output_variables: {
                    ...
                    sources: chunks.map((c) => ({ document: c.document_name, score: c.score })),
                },
            };
        });
        ...
        this.register('longterm_memory_fetch', async (config, variables, context) => {
            try {
                const result = await pool.query(`SELECT * FROM conversation_memories
                     WHERE agent_id = $1 AND visitor_id = $2
                     ORDER BY created_at DESC LIMIT $3`, [context.agent_id, context.visitor_id || '', config.top_k || 5]);
                return {
        ...
        this.register('longterm_memory_store', async (config, variables, context) => {
            try {
                ...
                await pool.query(`INSERT INTO conversation_memories (agent_id, visitor_id, memory_type, content, confidence)
                     VALUES ($1, $2, $3, $4, $5)`, [context.agent_id, context.visitor_id || '', config.memory_type || 'fact', content, config.confidence || 1.0]);
        ...
        this.register('tag_management', async (config, variables, context) => {
            ...
            try {
                if (context.conversation_id) {
                    if (action === 'add') {
                        await pool.query(`UPDATE conversations SET tags = array_append(tags, $1)
                             WHERE id = $2 AND NOT ($1 = ANY(tags))`, [tag, context.conversation_id]);
                    }
                    else {
                        await pool.query(`UPDATE conversations SET tags = array_remove(tags, $1) WHERE id = $2`, [tag, context.conversation_id]);
                    }
                }
                return {
        ...
        this.register('assign_agent', async (config, variables, context) => {
            try {
                if (context.conversation_id) {
                    ...
                    await pool.query(`UPDATE conversations SET assigned_to = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [assignTo, context.conversation_id]);
                }
                return {
                    output_variables: {
                        ...variables,
        ...
        this.register('close_conversation', async (config, variables, context) => {
            try {
                if (context.conversation_id) {
                    ...
                    await pool.query(`UPDATE conversations SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [newStatus, context.conversation_id]);
        ...
        this.register('visitor_property', async (config, variables) => {
            ...
            switch (config.operator) {
                case 'equals':
                    result = String(value).toLowerCase() === String(config.value).toLowerCase();
                    break;
                case 'not_equals':
                    result = String(value).toLowerCase() !== String(config.value).toLowerCase();
                    break;
                case 'contains':
                    result = String(value).toLowerCase().includes(String(config.value).toLowerCase());
                    break;
                case 'is_mobile':
                    result = String(value).toLowerCase() === 'mobile';
                    break;
                case 'is_desktop':
                    result = String(value).toLowerCase() === 'desktop';
                    break;
                case 'is_returning':
                    result = variables.is_returning === true;
                    break;
                default: result = false;
            }
            return {
...
//# sourceMappingURL=NodeExecutorRegistry.js.map