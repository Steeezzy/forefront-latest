...
export class ManualQnAService {
     ...
     */
    async createQnASource(agentId, name = 'Manual Q&A') {
        const result = await pool.query(`INSERT INTO knowledge_sources (agent_id, type, content, name, status)
     ...
     */
    async addQnAPair(sourceId, question, answer, category = null, language = 'en-IN') {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            // 1. Insert Q&A pair
            const result = await client.query(`INSERT INTO qna_pairs (knowledge_source_id, question, answer, category, language)
                 VALUES ($1, $2, $3, $4, $5)
                 RETURNING *`, [sourceId, question, answer, category, language]);
            ...
            await client.query(`INSERT INTO knowledge_vectors (source_id, content_chunk, embedding)
                 VALUES ($1, $2, $3)`, [sourceId, combinedText, JSON.stringify(embedding)]);
     ...
     */
    async updateQnAPair(qnaId, updates) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE qna_pairs
                 SET question = COALESCE($2, question),
                     answer = COALESCE($3, answer),
                     category = COALESCE($4, category),
                     updated_at = NOW()
                 WHERE id = $1
                 RETURNING *`, [qnaId, updates.question, updates.answer, updates.category]);
            const qna = result.rows[0];
            if (!qna)
            ...
            if (updates.question || updates.answer) {
                ...
                await client.query(`DELETE FROM knowledge_vectors
                     WHERE source_id = $1
                       AND content_chunk LIKE $2`, [qna.knowledge_source_id, `Question: %`]);
                // Generate new embedding
                const combinedText = `Question: ${qna.question}\nAnswer: ${qna.answer}`;
                const embedding = await generateEmbedding(combinedText);
                await client.query(`INSERT INTO knowledge_vectors (source_id, content_chunk, embedding)
                     VALUES ($1, $2, $3)`, [qna.knowledge_source_id, combinedText, JSON.stringify(embedding)]);
            }
            await client.query('COMMIT');
            return qna;
        }
    ...
    async deleteQnAPair(qnaId) {
        ...
        const combinedText = `Question: ${qna.question}\nAnswer: ${qna.answer}`;
        // Delete the vector
        await pool.query(`DELETE FROM knowledge_vectors WHERE source_id = $1 AND content_chunk = $2`, [qna.knowledge_source_id, combinedText]);
        // Delete the Q&A pair
     ...
     */
    async getQnAPairs(sourceId) {
        const result = await pool.query(`SELECT * FROM qna_pairs
     ...
     */
    async searchQnAPairs(sourceId, queryText) {
        const result = await pool.query(`SELECT *, similarity(question, $2) as score
             FROM qna_pairs
             WHERE knowledge_source_id = $1
               AND similarity(question, $2) > 0.3
             ORDER BY score DESC
             LIMIT 10`, [sourceId, queryText]);
        return result.rows;
    }
    /**
     * Increment usage count when a Q&A is used in a response
     */
    async incrementUsage(qnaId) {
