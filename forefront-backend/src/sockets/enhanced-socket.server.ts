...
export class EnhancedSocketServer {
  ...
  private onlineAgents: Map<string, Set<string>> = new Map(); // workspaceId -> Set of userIds
  ...
  private handleConnection(socket: Socket) {
    ...
    socket.on('send_message', async (data: {
      ...
      try {
        ...
        const conversation = await this.inboxService.getConversationById(
          data.conversationId,
          socket.data.user?.workspaceId || socket.data.workspaceId
        );
        ...
        if (data.senderType === 'visitor' && conversation) {
          ...
          this.enhancedRAG.resolveAIResponse(
            conversation.workspace_id,
            data.conversationId,
            data.content,
            { enableEscalation: true, escalationThreshold: 50 }
          )
    ...
    // Typing indicators
    socket.on('typing_start', (data: { conversationId: string; userType: 'visitor' | 'agent' }) => {
    ...
    socket.on('typing_stop', (data: { conversationId: string; userType: 'visitor' | 'agent' }) => {
      socket.to(data.conversationId).emit('typing_indicator', {
        conversationId: data.conversationId,
        userType: data.userType,
        isTyping: false,
    ...
    socket.on('get_copilot_suggestions', async (data: { conversationId: string }, callback: Function) => {
      try {
        if (socket.data.user?.workspaceId && socket.data.user?.userId) {
          const suggestions = await this.enhancedRAG.suggestReplies(
            data.conversationId,
            socket.data.user.workspaceId
          );
  ...
  private addOnlineAgent(workspaceId: string, userId: string) {
  ...
  private removeOnlineAgent(workspaceId: string, userId: string) {
  ...
  public getOnlineAgents(workspaceId: string): string[] {
