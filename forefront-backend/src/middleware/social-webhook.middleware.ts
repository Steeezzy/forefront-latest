...
import type { WebhookVerificationRequest } from '../types/social.types.js';
...
export async function verifyMetaWebhook(req: FastifyRequest, reply: FastifyReply) {
    ...
    const rawBody = (req as any).rawBody; // From fastify-raw-body
    if (!rawBody) {
 ...
 */
function handleMetaChallenge(req: FastifyRequest, reply: FastifyReply, expectedToken?: string) {
