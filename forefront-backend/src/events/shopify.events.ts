...
shopifyEventEmitter.on('shopify.order.fulfilled', async ({ order, storeId }) => {
    try {
        ...
        const contactRes = await pool.query(
            ...
            [order.email, workspaceId]
        );

        // Build tracking info
        const fulfillments = order.fulfillments || [];
        const trackingUrls = fulfillments
            .filter((f: any) => f.tracking_url)
            .map((f: any) => f.tracking_url);
        ...
        if (contactRes.rows.length > 0) {
            ...
            await pool.query(
                `INSERT INTO messages (conversation_id, workspace_id, content, sender_type, message_type, created_at)
                 VALUES ($1, $2, $3, 'system', 'note', NOW())`,
                [conversationId, workspaceId, trackingMsg]
            );
        }

        // Queue a notification for the workspace
        await pool.query(
            `INSERT INTO notifications (workspace_id, type, title, content, created_at, read)
             VALUES ($1, 'shopify', $2, $3, NOW(), false)`,
            [workspaceId, `Order ${order.name} fulfilled`, trackingMsg]
        );
        ...
        if (checkout.email) {
            ...
            if (contactRes.rows.length > 0) {
                await pool.query(
                    `INSERT INTO messages (conversation_id, workspace_id, content, sender_type, message_type, created_at)
                     VALUES ($1, $2, $3, 'system', 'note', NOW())`,
                    [contactRes.rows[0].id, workspaceId,
...
shopifyEventEmitter.on('shopify.app.uninstalled', async ({ storeId }) => {
    try {
        ...
        await pool.query(
            `INSERT INTO notifications (workspace_id, type, title, content, created_at, read)
             VALUES ($1, 'shopify', $2, $3, NOW(), false)`,
            [workspace_id, `Shopify app uninstalled`, `The Shopify app was uninstalled from ${shop_domain}. Reconnect to resume sync.`]
        );
