...
export class CustomerContextService {
     ...
     */
    async getContextByEmail(email: string, workspaceId: string): Promise<CustomerEcommerceContext | null> {
        // Find store for this workspace
        const storeRes = await pool.query(
            `SELECT id FROM shopify_configs WHERE workspace_id = $1 AND is_active = true LIMIT 1`, [workspaceId]
        );
        ...
        const custRes = await pool.query(
            `SELECT * FROM shopify_customers WHERE store_id = $1 AND email = $2 LIMIT 1`, [storeId, email]
        );
        if (custRes.rows.length === 0) return null;
        const customer = custRes.rows[0] as ShopifyCustomer;

        // Recent orders (last 5)
        const ordersRes = await pool.query(
            `SELECT * FROM shopify_orders WHERE store_id = $1 AND shopify_customer_id = $2 ORDER BY created_at DESC LIMIT 5`,
            [storeId, customer.shopify_id]
        );
        const recentOrders = ordersRes.rows as ShopifyOrder[];

        // Open issues (unfulfilled or pending payment)
        const issuesRes = await pool.query(
            `SELECT * FROM shopify_orders WHERE store_id = $1 AND shopify_customer_id = $2
       AND (fulfillment_status IN ('unfulfilled','partial') OR financial_status IN ('pending','authorized','partially_paid'))
       ORDER BY created_at DESC`,
            [storeId, customer.shopify_id]
        );
        const openIssues = issuesRes.rows as ShopifyOrder[];

        // LTV
        const ltvRes = await pool.query(
            `SELECT COALESCE(SUM(total_price), 0) as ltv FROM shopify_orders
       WHERE store_id = $1 AND shopify_customer_id = $2 AND financial_status = 'paid'`,
            [storeId, customer.shopify_id]
        );
        ...
        const checkoutRes = await pool.query(
            `SELECT * FROM shopify_abandoned_checkouts WHERE store_id = $1 AND email = $2 AND recovered_at IS NULL ORDER BY created_at DESC LIMIT 3`,
            [storeId, email]
        );
        const abandonedCheckouts = checkoutRes.rows as ShopifyAbandonedCheckout[];
     ...
     */
    async getContextByConversation(conversationId: string): Promise<CustomerEcommerceContext | null> {
        const convRes = await pool.query(
            `SELECT c.workspace_id, co.email
       FROM conversations c
       LEFT JOIN contacts co ON c.contact_id = co.id
       WHERE c.id = $1`,
            [conversationId]
        );
        if (convRes.rows.length === 0 || !convRes.rows[0].email) return null;
    ...
    formatContextForAI(ctx: CustomerEcommerceContext): string {
        ...
        if (ctx.recent_orders.length > 0) {
            const orderStrs = ctx.recent_orders.map(
                (o) => `${o.name} - ${currency} ${o.total_price} - ${o.fulfillment_status || 'unfulfilled'} / ${o.financial_status}`
            );
     ...
     */
    async searchOrders(query: string, storeId: string, limit = 10): Promise<ShopifyOrder[]> {
