...
export class InstagramService {
     ...
     * Send a text message.
     */
    async sendText(accountId: string, recipientId: string, text: string): Promise<string> {
     ...
     */
    async getSenderProfile(senderId: string, accessToken: string): Promise<{ name: string; profilePic?: string }> {
        try {
            const response = await fetch(`${API_BASE}/${senderId}?fields=name,profile_pic&access_token=${accessToken}`);
            if (!response.ok) return { name: 'Instagram User' };
    ...
    private async getAccessToken(accountId: string): Promise<string> {
        const result = await pool.query(
            `SELECT access_token FROM social_accounts WHERE account_id = $1 AND channel = 'instagram' AND connected = true`,
            [accountId]
        );
        if (result.rows.length === 0) throw new Error(`Instagram account ${accountId} not found`);
        return result.rows[0].access_token;
    }

    private async sendAPI(accountId: string, token: string, body: any): Promise<string> {
