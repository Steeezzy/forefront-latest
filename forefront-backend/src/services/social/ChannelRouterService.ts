...
export class ChannelRouterService {
     ...
     */
    async handleInbound(message: InboundSocialMessage): Promise<{
     ...
     */
    async handleDeliveryReceipt(receipt: SocialDeliveryReceipt): Promise<void> {
        await pool.query(
            `INSERT INTO social_message_statuses
     ...
     */
    async sendMessage(outbound: OutboundSocialMessage): Promise<string> {
        let externalId = '';
        ...
        switch (outbound.channel) {
            case 'whatsapp':
                if (outbound.content.type === 'text') {
                    ...
                    externalId = await this.whatsapp.sendMedia(
                        outbound.account_id, outbound.recipient_id,
                        outbound.content.type as any, outbound.content.media_url || '', outbound.content.caption
                    );
                }
                break;
            ...
            case 'instagram':
                if (outbound.content.type === 'text') {
                    ...
                    externalId = await this.instagram.sendMedia(
                        outbound.account_id, outbound.recipient_id, outbound.content.type as any, outbound.content.media_url || ''
                    );
                }
                break;
            ...
            case 'messenger':
                if (outbound.content.type === 'text') {
                    ...
                    externalId = await this.messenger.sendAttachment(
                        outbound.account_id, outbound.recipient_id, outbound.content.type as any, outbound.content.media_url || ''
                    );
                }
                break;
    ...
    async connectAccount(params: {
        ...
        const result = await pool.query(
            `INSERT INTO social_accounts (workspace_id, channel, account_id, account_name, access_token, webhook_secret, metadata, connected)
    ...
    private async findAccount(channel: string, accountId: string): Promise<SocialAccount | null> {
        const result = await pool.query(
            `SELECT * FROM social_accounts WHERE channel = $1 AND account_id = $2 AND connected = true`,
            [channel, accountId]
        );
    ...
    private async findOrCreateContact(
        ...
        const existing = await pool.query(
            `SELECT id FROM contacts WHERE workspace_id = $1 AND ${channelCol} = $2`,
        ...
        if (existing.rows.length > 0) {
            // Update last seen
            await pool.query(
                `UPDATE contacts SET last_seen_at = CURRENT_TIMESTAMP, name = COALESCE($1, name), avatar_url = COALESCE($2, avatar_url) WHERE id = $3`,
                [senderName || null, avatarUrl || null, existing.rows[0].id]
            );
        ...
        // Create new contact
        const result = await pool.query(
            `INSERT INTO contacts (workspace_id, name, avatar_url, ${channelCol})
       VALUES ($1, $2, $3, $4) RETURNING id`,
            [workspaceId, senderName || `${channel} User`, avatarUrl || null, senderId]
        );
