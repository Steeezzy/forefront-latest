...
export class WhatsAppService {
     ...
     * Send a text message.
     */
    async sendText(accountId: string, to: string, text: string, replyToId?: string): Promise<string> {
     ...
     */
    async getAccountDetails(phoneNumberId: string, accessToken: string): Promise<{ displayName: string; phoneNumber: string }> {
    ...
    private parseContent(msg: any): SocialMessageContent {
        switch (msg.type) {
            case 'text':
    ...
    private parseInteractiveReply(interactive: any): SocialMessageContent {
    ...
    private parseAttachments(msg: any): SocialAttachment[] {
        const attachments: SocialAttachment[] = [];
    ...
    private async getAccessToken(accountId: string): Promise<string> {
        const result = await pool.query(
            `SELECT access_token FROM social_accounts WHERE account_id = $1 AND channel = 'whatsapp' AND connected = true`,
            [accountId]
        );
        if (result.rows.length === 0) throw new Error(`WhatsApp account ${accountId} not found or disconnected`);
        return result.rows[0].access_token;
    }

    private async apiCall(phoneNumberId: string, token: string, body: any): Promise<any> {
    ...
    private parseError(data: any): Error {
