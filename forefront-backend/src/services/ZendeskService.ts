...
export class ZendeskService {
    private qnaService = new ManualQnAService();
    ...
    async importFromZendesk(
        ...
        const sourceRes = await pool.query(
            `INSERT INTO knowledge_sources (agent_id, type, content, url, name, status, metadata)
        ...
        try {
            ...
            const jobRes = await pool.query(
                `INSERT INTO processing_jobs (knowledge_source_id, job_type, status, total_items, started_at)
                 VALUES ($1, 'import', 'processing', $2, NOW())
                 RETURNING id`,
                [source.id, articles.length]
            );
            ...
            for (const article of articles) {
                try {
                    ...
                    if (article.title && plainBody.length > 10) {
                        await this.qnaService.addQnAPair(
                            source.id,
                            article.title,
                            plainBody,
                    ...
                    await pool.query(
                        `UPDATE processing_jobs SET processed_items = $2, progress = $3 WHERE id = $1`,
                        [jobId, processedCount, progress]
                    );
                } catch (err: any) {
     ...
     */
    private async fetchAllArticles(baseURL: string, authHeader: string) {
        const articles: any[] = [];
        let page = 1;
        let hasMore = true;
