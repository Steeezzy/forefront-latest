...
export class WidgetService {
  async getWidgetConfig(workspaceId: string) {
    const result = await query(
      'SELECT * FROM widget_configs WHERE workspace_id = $1',
      [workspaceId]
    );
  ...
  async getPublicWidgetConfig(workspaceId: string) {
    const result = await query(
      `SELECT 
       ...
       FROM widget_configs WHERE workspace_id = $1`,
      [workspaceId]
    );
  ...
  async updateWidgetConfig(workspaceId: string, data: z.infer<typeof widgetConfigSchema>) {
    const existing = await query(
      'SELECT id FROM widget_configs WHERE workspace_id = $1',
      [workspaceId]
    );
    ...
    const result = await query(
      `UPDATE widget_configs SET ${updates.join(', ')} WHERE workspace_id = $${paramIndex} RETURNING *`,
      values
    );
  ...
  private async createDefaultConfig(workspaceId: string) {
    const result = await query(
      `INSERT INTO widget_configs (workspace_id) VALUES ($1) RETURNING *`,
      [workspaceId]
    );
