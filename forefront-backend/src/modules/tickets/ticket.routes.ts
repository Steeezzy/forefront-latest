...
export async function ticketRoutes(app: FastifyInstance) {
    ...
    app.post('/:id/assign', async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
        try {
            const workspaceId = req.user?.workspaceId;
            const userId = req.user?.userId;
            if (!workspaceId || !userId) return reply.code(401).send({ error: 'Unauthorized' });
    ...
    app.post('/:id/comments', async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
        try {
            const workspaceId = req.user?.workspaceId;
            const userId = req.user?.userId;
            if (!workspaceId || !userId) return reply.code(401).send({ error: 'Unauthorized' });
    ...
    app.post('/from-conversation', async (req: FastifyRequest, reply: FastifyReply) => {
        try {
            const workspaceId = req.user?.workspaceId;
            const userId = req.user?.userId;
            if (!workspaceId || !userId) return reply.code(401).send({ error: 'Unauthorized' });
            ...
            const ticket = await ticketService.convertConversationToTicket(
                conversation_id,
                workspaceId,
                subject,
                userId
