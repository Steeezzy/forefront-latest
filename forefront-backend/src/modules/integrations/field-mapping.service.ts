...
const DEFAULT_MAPPINGS: Record<string, FieldMappingInput[]> = {
  hubspot: [
...
export class FieldMappingService {
   ...
   */
  async getMappings(workspaceId: string, integrationType: string): Promise<FieldMapping[]> {
    const { rows } = await pool.query(
      `SELECT * FROM integration_field_mappings
       WHERE workspace_id = $1 AND integration_type = $2
       ORDER BY is_required DESC, source_field`,
    ...
    try {
      ...
      await client.query(
        `DELETE FROM integration_field_mappings
         WHERE workspace_id = $1 AND integration_type = $2 AND is_required = false`,
        [workspaceId, integrationType]
      );
  ...
  async deleteMapping(workspaceId: string, mappingId: string): Promise<void> {
    await pool.query(
      `DELETE FROM integration_field_mappings
       WHERE id = $1 AND workspace_id = $2 AND is_required = false`,
   ...
   */
  async resetToDefaults(workspaceId: string, integrationType: string): Promise<FieldMapping[]> {
    await pool.query(
      `DELETE FROM integration_field_mappings
       WHERE workspace_id = $1 AND integration_type = $2`,
      [workspaceId, integrationType]
  ...
  private async seedDefaults(workspaceId: string, integrationType: string): Promise<void> {
