...
export class TeamService {
    ...
    try {
      ...
      const memberRes = await client.query(
        `INSERT INTO workspace_members 
         ...
         RETURNING *`,
        [workspaceId, userId, data.role, JSON.stringify(permissions), invitedBy]
      );
      ...
      await client.query(
        'UPDATE workspace_members SET metadata = $1 WHERE id = $2',
        [JSON.stringify({ inviteToken, email: data.email }), memberRes.rows[0].id]
      );
    ...
    try {
      ...
      const memberRes = await client.query(
        `SELECT * FROM workspace_members 
         WHERE metadata->>'inviteToken' = $1 AND status = 'pending'`,
        [token]
      );
  ...
  async removeMember(workspaceId: string, memberId: string) {
    const result = await query(
      'DELETE FROM workspace_members WHERE workspace_id = $1 AND id = $2 RETURNING *',
  ...
  async getUserPermissions(workspaceId: string, userId: string): Promise<Permissions> {
    const result = await query(
      'SELECT role, permissions FROM workspace_members WHERE workspace_id = $1 AND user_id = $2 AND status = $3',
