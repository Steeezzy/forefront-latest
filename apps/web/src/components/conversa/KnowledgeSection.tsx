...
export function KnowledgeSection() {
    ...
    return (
        <div>
            ...
            {/* Score Banner */}
            <div className="bg-[#ffffff] border border-gray-200 rounded-xl p-6 mb-6 flex items-center justify-between">
                <div className="flex items-center gap-6">
                    <div className="relative w-32 h-16 overflow-hidden">
                        <div className="absolute top-0 left-0 w-32 h-32 rounded-full border-[12px] border-gray-200 border-b-0 border-l-0 border-r-0 rotate-180" />
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">
                            {totalVectors > 0
                                ...
                                : "The score will appear once Conversa encounters questions it can't answer. Check back later!"
                            }
                        </p>
                    </div>
                </div>
                ...
                <Button
                    className="bg-blue-600 hover:bg-blue-500 text-gray-900 font-semibold"
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-gray-900 transition-colors"
                        >
                            <RefreshCw size={12} />
                            Refresh
                        </button>
                    ...
                    <div className="space-y-3">
                        {sources.map(source => {
                            ...
                            return (
                                <div
                                    key={source.id}
                                    className="bg-[#ffffff] border border-gray-200 rounded-xl p-4 hover:bg-[#f8fafc] transition-all group"
                                >
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-3 flex-1 min-w-0">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <h4 className="text-gray-900 font-medium text-sm truncate" title={source.url || source.name}>
            ...
            {/* Loading State */}
            {loading && (
                <div className="bg-[#ffffff] border border-gray-200 rounded-xl p-8 mb-8 flex items-center justify-center gap-3">
            ...
            {!loading && sources.length === 0 && (
                <div className="bg-[#ffffff] border border-gray-200 border-dashed rounded-xl p-8 mb-8 flex flex-col items-center justify-center text-center">
                    <div className="p-3 bg-blue-600/10 rounded-xl mb-3">
                        <BookOpen size={24} className="text-blue-400" />
                    </div>
                    <h4 className="text-gray-900 font-medium mb-1">No knowledge sources yet</h4>
            ...
            <div className="space-y-4 mb-8">
                {dataSummary.map((item, idx) => (
                    <div key={idx} className="bg-[#ffffff] border border-gray-200 rounded-xl p-4 flex items-center justify-between hover:bg-[#f8fafc] transition-colors group">
                        <div className="flex items-center gap-4">
                            <div className="p-2.5 bg-white/5 rounded-lg text-slate-400 group-hover:text-gray-900 transition-colors">
                                <item.icon size={20} />
                            </div>
                            <div>
                                <h4 className="text-gray-900 font-medium text-sm mb-0.5">{item.title}</h4>
                                <p className="text-slate-500 text-xs hidden md:block">{item.desc}</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-6">
                            <span className="text-gray-900 text-sm font-medium">{item.status}</span>
                            <Button 
                                variant="outline" 
                                size="sm" 
                                className="border-gray-200 text-gray-900 hover:bg-white/5 h-8"
