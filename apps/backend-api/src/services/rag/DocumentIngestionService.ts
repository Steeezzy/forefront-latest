...
interface ChunkingOptions {
    ...
    chunk_size: number;     // in tokens (approx chars / 4)
    chunk_overlap: number;  // in tokens
}
...
export class DocumentIngestionService {
    ...
    async ingestDocument(
        ...
        docId: string,
        rawText: string,
        options: ChunkingOptions = { strategy: 'fixed_size', chunk_size: 512, chunk_overlap: 50 }
    ): Promise<{ chunks_created: number; tokens_used: number }> {
        // 1. Update doc status to processing
        await pool.query(
            `UPDATE kb_documents SET status = 'processing', raw_text = $1, char_count = $2, token_count = $3 WHERE id = $4`,
            [rawText, rawText.length, this.embeddingService.estimateTokens(rawText), docId]
        );
        ...
        try {
            ...
            for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
                ...
                batchChunks.forEach((chunk, j) => {
                    ...
                    values.push(
                        ...
                        docId,
                        chunk.text,
                        `[${batchEmbeddings[j].embedding.join(',')}]`,
                        chunk.index,
                        chunk.char_start,
                        chunk.char_end,
                        chunk.token_count
                ...
                await pool.query(
                    `INSERT INTO document_chunks (kb_id, document_id, chunk_text, embedding, chunk_index, char_start, char_end, token_count, metadata)
            ...
            await pool.query(
                `UPDATE kb_documents SET status = 'indexed', chunk_count = $1 WHERE id = $2`,
                [chunks.length, docId]
            );
            ...
            await pool.query(
                `UPDATE kb_documents SET status = 'error', error_message = $1 WHERE id = $2`,
                [error.message, docId]
            );
            throw error;
        }
    ...
    private splitByFixedSize(text: string, chunkSize: number, overlap: number): Chunk[] {
    ...
    private splitBySentence(text: string, maxTokens: number, overlapTokens: number): Chunk[] {
    ...
    private splitByParagraph(text: string, maxTokens: number): Chunk[] {
