...
function detectAngrySentiment(message: string): boolean {
...
export class AutoReplyEngine {
  ...
  async processInboundMessage(params: {
    ...
    try {
      ...
      const convResult = await pool.query(
        `SELECT auto_reply_paused, agent_takeover, status FROM conversations WHERE id = $1`,
        [conversationId]
      );
      ...
      // Log the error
      await this.logAutoReply(workspaceId, conversationId, messageId, channel, visitorMessage,
        null, null, false, null, null, Date.now() - startTime, null, err.message);
  ...
  private async sendReply(
    ...
    if (channel === 'email') {
      ...
      await pool.query(
        `INSERT INTO messages (conversation_id, sender_type, content, message_type, metadata, ai_confidence)
         VALUES ($1, 'ai', $2, 'text', $3, 0)`,
        [conversationId, text, JSON.stringify({ channel: 'email', auto_reply: true, tone: settings.tone })]
      );
    ...
    const outbound: OutboundSocialMessage = {
      conversation_id: conversationId,
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO messages (conversation_id, sender_type, content, message_type, metadata, external_message_id, social_account_id, auto_reply_channel, tone_applied)
      ...
      // Update conversation
      await pool.query(
        `UPDATE conversations SET last_message_preview = $1, last_message_at = NOW(), updated_at = NOW(), ai_resolved = true
         WHERE id = $2`,
  ...
  private async escalate(
    ...
    await pool.query(
      `UPDATE conversations SET was_escalated = true, status = 'open', updated_at = NOW()
       WHERE id = $1`,
      [conversationId]
    );
    ...
    await pool.query(
      `INSERT INTO handoff_events (conversation_id, workspace_id, trigger, trigger_detail, status)
       VALUES ($1, $2, $3, $4, 'pending')`,
      [conversationId, workspaceId, trigger, reason]
    );
   ...
   */
  private async rewriteForTone(text: string, tone: string, channel: ChannelType): Promise<string> {
  ...
  private async logAutoReply(
    ...
    try {
      await pool.query(
        ...
        [workspaceId, conversationId, messageId, channel, visitorMessage, aiReply,
