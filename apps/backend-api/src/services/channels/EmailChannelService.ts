...
export class EmailChannelService {
   ...
   */
  async handleGmailCallback(code: string, state: string): Promise<EmailConnection> {
    ...
    const result = await pool.query(
      `INSERT INTO email_connections
       ...
       RETURNING *`,
      [workspaceId, emailAddress, tokens.access_token, tokens.refresh_token, expiresAt]
    );
   ...
   */
  async refreshGmailToken(connection: EmailConnection): Promise<string> {
    ...
    await pool.query(
      `UPDATE email_connections SET oauth_access_token = $1, oauth_token_expires_at = $2, updated_at = NOW()
       WHERE id = $3`,
   ...
   */
  async pollGmail(connection: EmailConnection): Promise<void> {
    try {
      ...
      const listRes = await fetch(
        `https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=20${afterParam}`,
      ...
      for (const msg of listData.messages) {
        try {
          ...
          if (parsed.from.includes(connection.email_address)) continue;
      ...
      if (latestTimestamp) {
        ...
        await pool.query(
          `UPDATE email_connections SET last_uid = $1, updated_at = NOW() WHERE id = $2`,
          [epochSeconds.toString(), connection.id]
        );
      }
    } catch (e: any) {
   ...
   */
  private parseGmailMessage(gmailMsg: any): ParsedEmail | null {
   ...
   */
  async processInboundEmail(connection: EmailConnection, email: ParsedEmail): Promise<void> {
    const workspaceId = connection.workspace_id;

    // 1. Find or create contact
    let contactResult = await pool.query(
      `SELECT id FROM contacts WHERE workspace_id = $1 AND email = $2 LIMIT 1`,
    ...
    if (contactResult.rows.length) {
      ...
      const newContact = await pool.query(
        `INSERT INTO contacts (workspace_id, email, name, channel, external_id)
         VALUES ($1, $2, $3, 'email', $4) RETURNING id`,
        [workspaceId, email.from, email.from_name || email.from, email.from]
      );
    ...
    if (email.in_reply_to || email.references) {
      const threadResult = await pool.query(
        `SELECT et.conversation_id FROM email_threads et
         WHERE et.workspace_id = $1 AND (
           et.thread_id = $2 OR et.thread_id = $3
         ) LIMIT 1`,
        [workspaceId, email.in_reply_to || '', email.message_id]
      );
      ...
      if (threadResult.rows.length) {
        ...
        for (const ref of refs) {
          const refResult = await pool.query(
            `SELECT conversation_id FROM email_threads WHERE workspace_id = $1 AND thread_id = $2 LIMIT 1`,
            [workspaceId, ref]
          );
    ...
    if (isNewThread || email.message_id) {
      await pool.query(
        `INSERT INTO email_threads (workspace_id, conversation_id, email_connection_id, thread_id, subject, from_address, to_address)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT DO NOTHING`,
        [workspaceId, conversationId!, connection.id, email.message_id, email.subject, email.from, email.to]
      );
    }

    // 4. Save message
    const msgResult = await pool.query(
      `INSERT INTO messages (conversation_id, sender_type, sender_id, content, message_type, metadata, contact_id)
   ...
   */
  private async createEmailConversation(workspaceId: string, contactId: string, email: ParsedEmail): Promise<string> {
    const result = await pool.query(
      `INSERT INTO conversations (workspace_id, channel, status, visitor_id, visitor_name, visitor_email, contact_id, subject)
   ...
   */
  async sendReply(conversationId: string, replyText: string): Promise<void> {
    // Get the thread info
    const threadResult = await pool.query(
      `SELECT et.*, ec.* FROM email_threads et
   ...
   */
  private async sendGmailReply(thread: any, replyText: string): Promise<void> {
  ...
  async startPolling(): Promise<void> {
    const result = await pool.query(
      `SELECT * FROM email_connections WHERE is_active = true`
    );
   ...
   */
  async getConnections(workspaceId: string): Promise<EmailConnection[]> {
    const result = await pool.query(
      `SELECT id, workspace_id, email_address, provider, use_ssl, is_active, created_at, updated_at
       FROM email_connections WHERE workspace_id = $1 ORDER BY created_at DESC`,
      [workspaceId]
    );
    return result.rows;
  }
