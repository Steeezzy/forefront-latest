...
interface WorkflowAction {
    type: string; // change_status, add_tag, remove_tag, assign_agent, send_reply, send_email, add_note
    value?: string;
    template?: string;
}
...
interface WorkflowConfig {
    ...
    assignment_method?: 'round_robin' | 'least_busy';
...
export class WorkflowEngine {
    ...
    async processEvent(
        workspaceId: string,
        triggerEvent: string,
        targetId: string, // conversation_id or ticket_id
       ...
       ORDER BY created_at ASC`,
        ...
        for (const workflow of result.rows) {
            try {
                const conditions: WorkflowConditions = workflow.conditions || {};
                const actions: WorkflowAction[] = workflow.actions || [];
                const config: WorkflowConfig = workflow.config || {};
                ...
                await pool.query(
                    `UPDATE workflows SET run_count = run_count + 1, last_run_at = CURRENT_TIMESTAMP WHERE id = $1`,
                    [workflow.id]
                );
    ...
    private async evaluateConditions(
        ...
        if (conditions.channel && target.channel !== conditions.channel) return false;
        if (conditions.status && target.status !== conditions.status) return false;
        if (conditions.priority && target.priority !== conditions.priority) return false;
        if (conditions.source && target.source !== conditions.source) return false;

        if (conditions.tags_contain && conditions.tags_contain.length > 0) {
            const tags: string[] = target.tags || [];
            if (!conditions.tags_contain.some((t: string) => tags.includes(t))) return false;
        }
        if (conditions.tags_exclude && conditions.tags_exclude.length > 0) {
            const tags: string[] = target.tags || [];
            if (conditions.tags_exclude.some((t: string) => tags.includes(t))) return false;
        }
    ...
    private async executeAction(
        ...
        switch (action.type) {
            ...
            case 'add_tag': {
                await pool.query(
                    `UPDATE conversations SET tags = array_append(tags, $1) WHERE id = $2 AND NOT ($1 = ANY(tags))`,
                    [action.value, targetId]
                ).catch(() => null);
                await pool.query(
                    `UPDATE tickets SET tags = array_append(tags, $1) WHERE id = $2 AND NOT ($1 = ANY(tags))`,
                    [action.value, targetId]
                ).catch(() => null);
            ...
            case 'remove_tag': {
                ...
                await pool.query(
                    `UPDATE tickets SET tags = array_remove(tags, $1) WHERE id = $2`,
                    [action.value, targetId]
                ).catch(() => null);
           ...
           VALUES ($1, 'bot', 'system', $2)`,
            ...
            case 'add_note': {
                ...
                await pool.query(
                    `INSERT INTO ticket_comments (ticket_id, author_type, author_name, content, is_internal)
           VALUES ($1, 'system', 'Workflow Automation', $2, true)`,
                    [targetId, action.value || 'Automated workflow action']
                ).catch(() => null);
     ...
     */
    private async getLeastBusyAgent(workspaceId: string): Promise<string | null> {
    ...
    private async logExecution(
        ...
        await pool.query(
            `INSERT INTO workflow_executions (workflow_id, trigger_data, actions_executed, status, error_message, target_id)
       VALUES ($1, $2, $3, $4, $5, $6)`,
            [
                workflowId,
        ...
        ).catch((err: any) => console.error('[WorkflowEngine] Log error:', err.message));
    }
