...
export class WorkflowScheduler {
    ...
    start() {
        ...
        this.runIdleChecks().catch((err) =>
            console.error('[WorkflowScheduler] Initial run error:', err.message)
        );
    }
        ...
        try {
         ...
         WHERE w.trigger_event = 'conversation_idle' AND w.is_active = true`
            ...
            for (const wf of workflows.rows) {
                ...
                const idleConvos = await pool.query(
           ...
           LIMIT 50`,
                    [wf.workspace_id, idleMinutes, wf.id]
                );
    ...
    private async checkIdleTickets() {
        try {
         ...
         WHERE w.trigger_event = 'ticket_idle' AND w.is_active = true`
           ...
           LIMIT 50`,
                    [wf.workspace_id, idleDays, wf.id]
                );
