...
export class MetaOAuthService {
     ...
     */
    async getAuthorizationUrl(projectId: string, channel: 'instagram' | 'messenger'): Promise<string> {
     ...
     */
    async handleCallback(code: string, stateBase64: string): Promise<{ pages: MetaPage[]; projectId: string; channel: string }> {
        // 1. Validate State + Nonce
        let stateObj: { projectId: string; channel: string; nonce: string };
        try {
     ...
     */
    async exchangePageToken(pageId: string, userAccessToken: string): Promise<string> {
        const response = await fetch(`${GRAPH_URL}/${pageId}?fields=access_token&access_token=${userAccessToken}`);
        const data: any = await response.json();
        if (!response.ok) throw new Error(`Page token exchange failed: ${data.error?.message}`);
     ...
     */
    async getInstagramAccountForPage(pageId: string, pageToken: string): Promise<{ igAccountId: string; username: string }> {
        const response = await fetch(`${GRAPH_URL}/${pageId}?fields=instagram_business_account{id,username}&access_token=${pageToken}`);
        const data: any = await response.json();
        if (!response.ok) throw new Error(`Fetch IG account failed: ${data.error?.message}`);
    ...
    private async exchangeCodeForToken(code: string): Promise<string> {
        ...
        const data: any = await response.json();
        if (!response.ok) throw new Error(`Code exchange failed: ${data.error?.message}`);
        return data.access_token;
    }

    private async getLongLivedToken(shortToken: string): Promise<string> {
        const params = new URLSearchParams({
        ...
        const response = await fetch(`${GRAPH_URL}/oauth/access_token?${params.toString()}`);
        const data: any = await response.json();
        if (!response.ok) throw new Error(`Long-lived token upgrade failed: ${data.error?.message}`);
        return data.access_token;
    }

    private async getUserPages(userToken: string): Promise<MetaPage[]> {
        ...
        const data: any = await response.json();
        if (!response.ok) throw new Error(`Fetch pages failed: ${data.error?.message}`);
        return data.data || [];
    }
}
