  ...
  for (const entry of entries) {
    ...
    for (const event of messagingEvents) {
      ...
      await crmAutomationService.autoSyncContact(workspaceId, {
        email: '', // Facebook doesn't provide email in webhook payload
        name: parsed.senderName || 'Facebook User',
        phone: '',
        source: 'facebook'
      }).catch(err => log.error(`[CRM Sync] Facebook auto-sync failed: ${err.message}`));
      ...
      }).catch(err => log.error(`[IntegrationEvents] Facebook event trigger failed: ${err.message}`));
