    ...
    for (const change of changes) {
      ...
      for (const message of messages) {
        ...
        await crmAutomationService.autoSyncContact(workspaceId, {
          email: '', // WhatsApp doesn't provide email in webhook payload
          name: parsed.senderName || 'WhatsApp User',
          phone: parsed.senderPhone,
          source: 'whatsapp'
        }).catch(err => log.error(`[CRM Sync] WhatsApp auto-sync failed: ${err.message}`));
        ...
        }).catch(err => log.error(`[IntegrationEvents] WhatsApp event trigger failed: ${err.message}`));
