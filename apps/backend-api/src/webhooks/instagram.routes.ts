  ...
  for (const entry of entries) {
    ...
    for (const event of messagingEvents) {
      ...
      await crmAutomationService.autoSyncContact(workspaceId, {
        email: '', // Instagram doesn't provide email in webhook payload
        name: parsed.senderUsername || 'Instagram User',
        phone: '',
        source: 'instagram'
      }).catch(err => log.error(`[CRM Sync] Instagram auto-sync failed: ${err.message}`));
      ...
      }).catch(err => log.error(`[IntegrationEvents] Instagram event trigger failed: ${err.message}`));
