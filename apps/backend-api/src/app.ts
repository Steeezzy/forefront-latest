...
app.register(cookie, {
  secret: env.COOKIE_SECRET, // Use env in prod
  parseOptions: {}
});
...
import { managedAgentRoutes } from './modules/agent/managed-agents.routes.js';
...
app.register(workspaceRoutes, { prefix: '/api/workspaces' }); // Fix for Issue 1
app.register(orchestratorRoutes, { prefix: '/api/orchestrator' });

// Managed Agents (Issue 8)
app.register(managedAgentRoutes, { prefix: '/api/managed-agents' });

// VAPI voice infrastructure: assistant sync, server URL/webhooks, and tool calls.
app.register(vapiRoutes);

// Hermes MCP Bridge — /api/mcp/*
// Protected by QESTRON_ADMIN_TOKEN, consumed exclusively by the Qestron MCP Server
// which Hermes uses to orchestrate Qestron tasks autonomously.
...
// Website builder
app.register(websiteRoutes, { prefix: '/api/websites' });
app.register(storeRoutes, { prefix: '/api/store' });

// Phase 6: Support Portal — internal staff tools for tickets, clients, onboarding flows
