...
export async function flowRoutes(app: FastifyInstance) {
    ...
    app.post('/', { preHandler: [authenticate] }, async (req: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            // Resolve workspace_id from the agent (flows table requires it NOT NULL)
            const agentRow = await pool.query(
                `SELECT workspace_id FROM agents WHERE id = $1 LIMIT 1`,
                [body.agentId]
            );
            if (agentRow.rows.length === 0) {
                return reply.status(404).send({ error: 'Agent not found' });
            }
            const workspaceId = agentRow.rows[0].workspace_id;

            const result = await pool.query(
                `INSERT INTO flows (agent_id, workspace_id, name) VALUES ($1, $2, $3) RETURNING *`,
                [body.agentId, workspaceId, body.name || 'Untitled Flow']
            );
    ...
    app.post('/from-template', { preHandler: [authenticate] }, async (req: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            // Resolve workspace_id from the agent
            const agentRow = await pool.query(
                `SELECT workspace_id FROM agents WHERE id = $1 LIMIT 1`,
                [agentId]
            );
            ...
            const result = await pool.query(
                `INSERT INTO flows (agent_id, workspace_id, name, description, trigger_type, is_active, nodes, edges)
