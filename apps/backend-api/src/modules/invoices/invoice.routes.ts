...
export async function invoiceRoutes(app: FastifyInstance) {
  ...
  // Generate stripe payment link (Issue 5)
  app.post('/:id/payment-link', { preHandler: [authenticate] }, async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = req.params as { id: string };
      const link = await invoiceService.createPaymentLink(id);
