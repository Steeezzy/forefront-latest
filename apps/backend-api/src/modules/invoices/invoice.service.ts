...
export class InvoiceService {
  ...
  async createPaymentLink(invoiceId: string) {
    ...
    const { StripeService } = await import('../billing/services/StripeService.js');
    const stripeService = new StripeService();

    const { id, url } = await stripeService.createInvoicePaymentLink(
      invoiceId,
    ...
    await pool.query(
      `UPDATE invoices 
       SET stripe_payment_link_id = $1, stripe_payment_link_url = $2, updated_at = NOW()
       WHERE id = $3`,
      [id, url, invoiceId]
    );
