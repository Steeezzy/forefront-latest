...
import type { OutboundSocialMessage, SocialChannel } from '../../types/social.types.js';
import type { ChannelType } from '../../services/channels/ChannelSettingsService.js';
...
export async function socialRoutes(fastify: FastifyInstance) {
    ...
    async function triggerAutoReply(
        channel: ChannelType,
        result: { conversation_id: string; contact_id: string; message_id: string },
        accountId: string,
        senderId: string,
        messageText: string,
        log: any
        ...
        try {
            ...
            const acct = await pool.query(
                `SELECT workspace_id FROM social_accounts WHERE account_id = $1 AND channel = $2 LIMIT 1`,
                [accountId, channel]
            );
            ...
            }).catch((e: any) => log.error(`[AutoReply][${channel}] ${e.message}`));
        } catch (e: any) {
    ...
    fastify.register(async function privateApi(api) {
        ...
        api.delete('/accounts/:id', async (req, reply) => {
            ...
            await pool.query(
                `UPDATE social_accounts SET connected = false, updated_at = NOW() WHERE id = $1 AND workspace_id = $2`,
                [id, workspaceId]
            );
        ...
        api.get('/accounts/meta/auth-url', async (req, reply) => {
            const workspaceId = (req as any).user?.workspaceId;
            const { channel } = req.query as { channel: 'instagram' | 'messenger' };
            if (!channel || !workspaceId) return reply.code(400).send({ error: 'Missing channel or workspaceId' });
        ...
        api.post('/accounts/meta/connect-page', async (req, reply) => {
            const workspaceId = (req as any).user?.workspaceId;
            if (!workspaceId) return reply.code(401).send({ error: 'No workspace' });
            const { pageId, userAccessToken, channel, pageName } = req.body as {
                pageId: string; userAccessToken: string; channel: 'instagram' | 'messenger'; pageName: string;
            };
        ...
        api.post('/accounts/whatsapp/connect', async (req, reply) => {
            const workspaceId = (req as any).user?.workspaceId;
            if (!workspaceId) return reply.code(401).send({ error: 'No workspace' });
            const { phoneNumberId, accessToken, webhookSecret } = req.body as {
                phoneNumberId: string; accessToken: string; webhookSecret: string;
            };
        ...
        api.post('/send', async (req, reply) => {
            try {
                const externalId = await channelRouterService.sendMessage(req.body as OutboundSocialMessage);
