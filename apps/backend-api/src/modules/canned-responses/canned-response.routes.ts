...
export async function cannedResponseRoutes(fastify: FastifyInstance) {
    ...
    try {
      ...
      const result = await query(
        `INSERT INTO canned_responses 
         ...
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [workspaceId, data.shortcut, data.title, data.content, data.category || null, userId]
      );
  ...
  fastify.patch('/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    try {
      ...
      const result = await query(
        `UPDATE canned_responses SET ${updates.join(', ')} WHERE id = $${paramIndex} AND workspace_id = $${paramIndex + 1} RETURNING *`,
        values
      );
  ...
  fastify.delete('/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    try {
      ...
      const result = await query(
        'DELETE FROM canned_responses WHERE id = $1 AND workspace_id = $2 RETURNING *',
  ...
  fastify.post('/:id/use', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    try {
      ...
      await query(
        'UPDATE canned_responses SET use_count = use_count + 1 WHERE id = $1 AND workspace_id = $2',
        [request.params.id, workspaceId]
      );
