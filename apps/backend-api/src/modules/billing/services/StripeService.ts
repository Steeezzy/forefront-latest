...
import { getPlanById, getAllPlans, getPlanStripePriceId, RECHARGE_PACKS, type PlanDefinition } from '../plans.js';
...
export class StripeService implements IBillingProvider {
    ...
    async createInvoicePaymentLink(invoiceId: string, amount: number, invoiceNumber: string, customerEmail?: string): Promise<{ id: string; url: string }> {
        ...
        const link = await this.stripe.paymentLinks.create({
            ...
            after_completion: {
                type: 'redirect',
                redirect: {
                    url: `${env.FRONTEND_URL}/panel/invoices/${invoiceId}?payment=success`,
                },
            },
        });
    ...
    async createCreditRechargeLink(workspaceId: string, packId: string, email: string): Promise<{ sessionId: string; url: string }> {
        const pack = RECHARGE_PACKS.find(p => p.id === packId);
        if (!pack) throw new Error(`Invalid recharge pack: ${packId}`);
        ...
        const session = await this.stripe.checkout.sessions.create({
            ...
            line_items: [{
                price_data: {
                    currency: 'inr',
                    product_data: { name: `Qestron ${pack.name} (${pack.credits.toLocaleString()} Credits)` },
                    unit_amount: Math.round(pack.price * 100),
                },
                quantity: 1,
            }],
            success_url: `${env.FRONTEND_URL}/panel/settings/billing?recharge=success`,
            cancel_url: `${env.FRONTEND_URL}/panel/settings/billing?recharge=canceled`,
            metadata: {
        ...
        switch (event.type) {
            case 'checkout.session.completed': {
                ...
                if (type === 'invoice_payment') {
                    ...
                    if (invoiceId) {
                        const { invoiceService } = await import('../../invoices/invoice.service.js');
                        await invoiceService.markAsPaid(invoiceId);
                        console.log(`[Stripe] Invoice paid: ${invoiceId}`);
                    }
                    break;
                }
                ...
                if (type === 'credit_recharge') {
                    ...
                    const packIdStr = session.metadata?.packId;
                    
                    if (workspaceId && creditsStr) {
                        const amountPaid = parseFloat(amountStr || '0');
                        const creditsToAdd = parseFloat(creditsStr);
                        
                        await pool.query(
                            `INSERT INTO credit_purchases (workspace_id, stripe_session_id, pack_id, credits, amount_paid)
                             VALUES ($1, $2, $3, $4, $5)`,
                            [workspaceId, session.id, packIdStr, creditsToAdd, amountPaid]
                        );

                        await pool.query(
                            `UPDATE workspaces SET recharge_credits = COALESCE(recharge_credits, 0) + $2 WHERE id = $1`,
                            [workspaceId, creditsToAdd]
                        );
                        console.log(`[Stripe] Credit Recharge Complete: workspace=${workspaceId} credits=${creditsToAdd}`);
                    }
                    break;
                }
