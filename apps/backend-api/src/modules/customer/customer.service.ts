...
Please analyze and return ONLY valid JSON with this structure:
{
  ...
  /**
   * Update deal stage for a customer
   */
  async updateDealStage(workspaceId: string, profileId: string, stage: string) {
    const result = await pool.query(
      `UPDATE customer_profiles
       SET deal_stage = $1, updated_at = NOW()
       WHERE id = $2 AND workspace_id = $3
       RETURNING *`,
      [stage, profileId, workspaceId]
    );
    ...
    // Log activity
    await pool.query(
      `INSERT INTO customer_activities (workspace_id, customer_id, activity_type, title, description)
       VALUES ($1, $2, 'stage_change', 'Stage Updated', $3)`,
      [workspaceId, profileId, `Deal stage updated to ${stage}`]
    );
  ...
  /**
   * Get full customer detail with interaction history and activities
   */
  async getCustomerDetail(workspaceId: string, profileId: string) {
    const profile = await this.getProfile(profileId);
    if (profile.workspace_id !== workspaceId) {
      throw new Error('Unauthorized or profile not in workspace');
    }

    const interactions = await this.getHistory(profileId);
    
    // Check if customer_activities table exists (it should based on migration 086)
    let activities = [];
    try {
      ...
      console.warn('customer_activities table might be missing:', e);
    }
