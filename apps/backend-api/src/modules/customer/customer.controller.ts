...
export class CustomerController {
  ...
  /**
   * Update customer deal stage
   * PUT /api/customers/:workspaceId/:profileId/stage
   */
  async updateDealStage(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { workspaceId, profileId } = request.params as {
        workspaceId: string;
        profileId: string;
      ...
      const updated = await customerService.updateDealStage(
        workspaceId,
        profileId,
        stage
      );
   ...
   * GET /api/customers/:workspaceId/:profileId/detail
   */
