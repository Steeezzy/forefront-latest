...
export class RagService {
    async resolveAIResponse(workspaceId: string, userMessage: string): Promise<string> {
        ...
        const { rows: agents } = await pool.query(
            `SELECT system_prompt, tone FROM agents WHERE workspace_id = $1 AND is_active = true LIMIT 1`,
            [workspaceId]
        );
