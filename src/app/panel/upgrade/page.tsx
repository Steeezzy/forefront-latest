...
import { customerServicePlans, conversaAiPlans, flowsPlans } from "@/components/upgrade/PricingData";

type TabOption = 'customer-service' | 'conversa-ai' | 'flows';

export default function UpgradePage() {
    const [activeTab, setActiveTab] = useState<TabOption>('conversa-ai'); // Default per screenshot

    const renderPlans = () => {
        switch (activeTab) {
            case 'customer-service': return customerServicePlans;
            case 'conversa-ai': return conversaAiPlans;
            case 'flows': return flowsPlans;
            default: return conversaAiPlans;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] text-gray-900 flex flex-col font-sans">
            {/* Header */}
            <header className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-[#f8fafc] sticky top-0 z-50">
                <div className="flex items-center gap-3 cursor-pointer group hover:bg-white/5 p-2 rounded-lg transition-colors" onClick={() => window.history.back()}>
                    <span className="text-slate-400 group-hover:text-gray-900 transition-colors">✕</span>
                    <h1 className="text-sm font-bold tracking-wide">Upgrade</h1>
                </div>
                <div className="flex items-center gap-4 text-sm font-semibold text-slate-400 hover:text-gray-900 transition-colors">
            ...
            <main className="flex-1 overflow-x-hidden flex flex-col items-center py-16 px-6">
                ...
                <div className="text-center mb-12 max-w-2xl px-4">
                    ...
                    <p className="text-slate-400 text-sm md:text-base">
                        Bundle Customer service, Conversa AI Agent, Flows and Campaigns into one powerful subscription.
                    </p>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-20 max-w-4xl justify-center px-4 w-full">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === 'customer-service'
                                ...
                                : "border-gray-200 hover:border-white/20 bg-[#ffffff]"
                        )}
                    >
                        <MessageSquare
                            size={24}
                        ...
                        <div className="flex flex-col gap-0.5">
                            <span className={cn(
                                ...
                                activeTab === 'customer-service' ? "text-gray-900" : "text-slate-300"
                    ...
                    {/* Tab 2: Conversa AI Agent */}
                    <button
                        onClick={() => setActiveTab('conversa-ai')}
                        className={cn(
                            "flex items-center gap-4 flex-1 p-5 rounded-xl border text-left transition-all",
                            activeTab === 'conversa-ai'
                                ? "border-blue-500 bg-blue-900/10 shadow-[0_0_20px_rgba(59,130,246,0.15)]"
                                : "border-gray-200 hover:border-white/20 bg-[#ffffff]"
                        )}
                    >
                        <Bot
                            size={24}
                            className={cn(activeTab === 'conversa-ai' ? "text-blue-500" : "text-slate-400")}
                        />
                        <div className="flex flex-col gap-0.5">
                            <span className={cn(
                                "font-semibold text-sm",
                                activeTab === 'conversa-ai' ? "text-gray-900" : "text-slate-300"
                            )}>Conversa AI Agent</span>
                            <span className="text-xs text-slate-500 line-clamp-2 leading-relaxed">
                                Solve up to 70% of customer problems with Conversa AI - the AI support agent
                            </span>
                        </div>
                    </button>
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-0.5">
                            <span className={cn(
                                "font-semibold text-sm",
                                activeTab === 'flows' ? "text-gray-900" : "text-slate-300"
                            )}>Flows</span>
                            <span className="text-xs text-slate-500 line-clamp-2 leading-relaxed">
                ...
                <div className="w-full max-w-7xl px-4 pb-20 overflow-x-auto pb-4 custom-scrollbar">
                    <div className="flex items-stretch gap-6 min-w-max pb-4 justify-center">
                        ...
                        {/* Recommendation Card */}
                        <div className="w-[300px] flex-shrink-0 bg-[#ffffff] border border-gray-200 rounded-2xl p-8 flex flex-col items-center justify-start text-center h-[500px]">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 mb-4">Not sure which plan to choose?</h3>
