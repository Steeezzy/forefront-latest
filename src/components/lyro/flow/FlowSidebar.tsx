...
export function FlowSidebar() {
    ...
    return (
        <div className="w-80 bg-[#18181b] border-l border-white/5 h-full flex flex-col z-10">
            {/* Tabs */}
...
function DraggableItem({ type, label, icon, color, onDragStart }: any) {
    const bgColor = color === 'blue' ? 'bg-[#3b82f6]' : color === 'orange' ? 'bg-[#f97316]' : 'bg-slate-700';

    return (
        <div
            className="flex flex-col gap-2 p-3 bg-black/20 border border-white/5 rounded-lg cursor-grab hover:border-blue-500/50 hover:bg-black/40 transition-all group"
            draggable
            onDragStart={(e) => onDragStart(e, type, label)}
        >
            <div className={`w-8 h-8 rounded-md flex items-center justify-center text-white ${bgColor}`}>
                {icon}
            </div>
