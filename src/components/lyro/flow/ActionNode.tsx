...
export function ActionNode({ data }: { data: { label: string, type: string } }) {
    ...
    return (
        <div className="bg-[#18181b] rounded-lg shadow-sm border border-white/10 w-48 overflow-hidden group hover:border-blue-500 transition-colors">
            <Handle
                type="target"
                position={Position.Top}
                className="w-3 h-3 bg-[#18181b] border-2 border-white/20 group-hover:border-blue-500"
            />

            <div className="p-3 flex items-center gap-3">
                <div className={`w-8 h-8 rounded-md flex items-center justify-center text-white ${getBgColor()}`}>
                    {getIcon()}
                </div>
