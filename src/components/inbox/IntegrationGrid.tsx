...
export function IntegrationGrid() {
    return (
        <div className="mt-10">
            <p className="text-slate-400 text-sm mb-6">You can also integrate Questron with other apps to keep everything in one place!</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {integrations.map((item, idx) => (
                    <Link
                        key={idx}
                        href={item.href}
                        className="group bg-[#f8fafc] border border-gray-200 rounded-xl p-5 flex flex-col items-start hover:bg-[#f8fafc] hover:border-gray-200 transition-all"
                    >
                        <div className={`w-10 h-10 rounded-xl ${item.bgColor} flex items-center justify-center mb-3`}>
                            <item.icon className={`w-5 h-5 ${item.color}`} />
                        </div>
                        <h4 className="text-gray-900 font-semibold mb-1.5">{item.title}</h4>
