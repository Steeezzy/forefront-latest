...
export function ManageQnAModal({ isOpen, onClose }: ManageQnAModalProps) {
    ...
    return (
        <>
            ...
            {/* Modal */}
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-3xl max-h-[80vh] bg-[#f8fafc] border border-zinc-800 rounded-xl z-50 flex flex-col shadow-2xl">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                    <div className="flex items-center gap-3">
                        <MessageSquare className="w-5 h-5 text-blue-500" />
                        <h2 className="text-lg font-bold text-gray-900">Manage Q&A Pairs</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {/* Add New Form */}
                    {showAddForm && (
                        <div className="mb-6 p-4 bg-white border border-zinc-700 rounded-lg">
                            <h4 className="text-sm font-medium text-gray-900 mb-3">Add New Q&A</h4>
                            <input
                                type="text"
                                value={newQuestion}
                                onChange={(e) => setNewQuestion(e.target.value)}
                                placeholder="Question"
                                className="w-full px-3 py-2 mb-3 rounded-lg bg-zinc-800 border border-zinc-700 text-gray-900 placeholder-zinc-500 focus:outline-none focus:border-blue-500 text-sm"
                            />
                            <textarea
                                value={newAnswer}
                                ...
                                placeholder="Answer"
                                rows={3}
                                className="w-full px-3 py-2 mb-3 rounded-lg bg-zinc-800 border border-zinc-700 text-gray-900 placeholder-zinc-500 focus:outline-none focus:border-blue-500 text-sm resize-none"
                            />
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <h4 className="text-gray-900 font-medium mb-1">No Q&A pairs yet</h4>
                        ...
                        <div className="space-y-3">
                            {pairs.map(pair => (
                                <div
                                    key={pair.id}
                                    className="p-4 bg-white border border-zinc-800 rounded-lg hover:border-zinc-700 transition-colors"
                                >
                                    {editingId === pair.id ? (
                                        <div className="space-y-3">
                                            <input
                                                ...
                                                className="w-full px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-gray-900 focus:outline-none focus:border-blue-500 text-sm"
                                            />
                                            <textarea
                                                value={editAnswer}
                                                onChange={(e) => setEditAnswer(e.target.value)}
                                                rows={3}
                                                className="w-full px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-gray-900 focus:outline-none focus:border-blue-500 text-sm resize-none"
                                            />
                                            <div className="flex justify-end gap-2">
                                                <Button
                                                    variant="ghost"
